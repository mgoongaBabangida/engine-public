...
//------------------------------------------------------
class DLL_OPENGL_ASSETS eModel: public IAnimatedModel
{
...
public:
  ...
  virtual ~eModel();
	...
	virtual void DrawInstanced(int32_t _program, int32_t _instances) override;
  ...
  virtual std::vector<const IAnimation*>	GetAnimations() const override;
	...
	const std::vector<SkeletalAnimation>&	Animations()		const { return m_animations; }
	...
	const eMesh* GetMeshByName(const std::string& _name) const;

	// animations are lost
	std::vector<std::unique_ptr<IModel>> Split() override;
