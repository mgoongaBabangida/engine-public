...
eBezierRender::eBezierRender(const std::string& vS, const std::string& fS, const std::string& tS1, const std::string& tS2)
{
  ...
  m_rotationMatrixLoc = glGetUniformLocation(m_bezier_shader.ID(), "rotationMatrix");

  glm::mat2 rotationMatrix = glm::mat2(1.0f);
  glUniformMatrix2fv(m_rotationMatrixLoc, 1, GL_FALSE, glm::value_ptr(rotationMatrix));
}
