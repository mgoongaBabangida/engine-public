...
public:
	...
	void Initialize(Texture* diffuse, Texture* specular, Texture* normal = nullptr, Texture* heightMap = nullptr,
									bool spreed_texture = true, float _height_scale = 1.0f, float _max_height = 1.0f, float _min_height = 0.0f, int32_t normal_sharpness = 10, unsigned int _tessellation_coef = 16, unsigned int _cpu_lods = 1);
	
	void AddOrUpdate(glm::ivec2 _pos, const TessellationRenderingInfo&, const Texture* _diffuse, const Texture* heightMap = nullptr,
									 bool spreed_texture = true, int32_t normal_sharpness = 10, bool _apply_normal_blur = false, unsigned int _tessellation_coef = 16, unsigned int _cpu_lods = 1);

	void SetAABBtoChunk(extremDots _dots, glm::ivec2 _pos);
  ...
  virtual std::vector<const IMesh*>				GetMeshes() const;
	...
	virtual std::vector<extremDots>								GetExtremDotsOfMeshes() const override;
