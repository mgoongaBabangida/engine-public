...
eLeafRender* eRenderManager::LeafRender() { return m_leafRender.get(); }
eDecalRender* eRenderManager::DecalRender() { return m_decalRender.get(); }
...
//-----------------------------------------------------------------------------------------------------
std::vector<std::shared_ptr<IParticleSystem> > eRenderManager::GetParticleSystems()
{
	return m_particleRender->GetParticleSystems();
}
...
void eRenderManager::Initialize(eModelManager& modelManager, eTextureManager& texManager, const std::string& _folderPath)
{
	...
	m_waterRender.reset(new eWaterRender(modelManager.CreateFromBrush("square"),
	...
	m_screenRender.reset(new eScreenRender(*(texManager.Find("TcubeSkyWater1")),
																					...
																					folderPath + "LuminanceCs.glsl",
																					folderPath + "UI_vert.glsl",
																					folderPath + "UI_frag.glsl",
																					folderPath + "msdf.glsl",
																					folderPath + "SolidColorfragment.glsl"));
	shader_lambda(m_screenRender.get());

	//PhongRender
	m_phongRender.reset(new ePhongRender(folderPath + "VertexShader.glsl", folderPath + "WaveVertexShader.glsl", folderPath + "PhongFragmentShader.glsl",
																			 modelManager.CloneTerrain("simple"), const_cast<Texture*>(texManager.Find("TSpanishFlag0_s")))); //@todo const cast !
	shader_lambda(m_phongRender.get());
	//Shadow Render
	m_shadowRender.reset(new eShadowRender(folderPath + "VertexShader.glsl",
	...
	//eOutlineRender
	m_outlineRender.reset(new eOutlineRender(folderPath + "VertexShader.glsl", folderPath + "StencilFragmentShader.glsl"));
	shader_lambda(m_outlineRender.get());
	//eSkyNoiseRender
	m_skynoiseRender.reset(new eSkyNoiseRender(modelManager.CreateFromBrush("square"),
	...
	// Particle Renderer
	m_particleRender.reset(new eParticleRender(modelManager.FindBrush("square"),
	...
	//PBR
	m_pbrRender.reset(new ePBRRender(folderPath + "VertexShader.glsl",
																	 folderPath + "PBRFragmentShader.glsl"));
	...
	// Mesh line
	m_meshlineRender.reset(new eMeshLineRender(folderPath + "VertexShader.glsl", folderPath + "StencilFragmentShader.glsl"));
	...
	//Area lights
	m_area_lights.reset(new eAreaLightsOnlyRender(folderPath + "VertexShader.glsl", folderPath + "AreaLightOnlyFS.glsl"));
	...
	//Leaf rendering
	m_leafRender.reset(new eLeafRender(folderPath + "LeafVertexShader.glsl", folderPath + "LeafFragment.glsl"));
	shader_lambda(m_leafRender.get());

	//Decal rendering
	m_decalRender.reset(new eDecalRender(folderPath + "PostProcessingVertexShader.glsl", folderPath + "DecalFragment.glsl"));
	shader_lambda(m_decalRender.get());
}
...
bool eRenderManager::SetUniformData(const std::string& _renderName, const std::string& _uniformName, const UniformData& _data)
{
	...
	if (shader_lambda(m_volumetricRender.get()))
		return true;
	if (shader_lambda(m_leafRender.get()))
		return true;
	if (shader_lambda(m_decalRender.get()))
		return true;
	return false;
}
