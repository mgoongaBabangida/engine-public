...
, m_cube(new CubeMesh)
{
  ...
  GLuint rotationMatrixLoc = glGetUniformLocation(m_brdf_shader.ID(), "rotationMatrix");
...
void eIBLRender::RenderIBLMap(const Camera& camera, GLuint _irr_id)
{
  ...
  m_irr_shader.SetUniformData("projection", captureProjection);

  for (unsigned int i = 0; i < 6; ++i)
  {
    m_irr_shader.SetUniformData("view", m_captureViews[i]);
