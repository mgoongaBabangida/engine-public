...
//----------------------------------------------------------------------------------------------
class DLL_OPENGL_ASSETS Brush: public MeshBase
{
public:
	enum class RenderMode { DEFAULT, WIREFRAME };

	explicit Brush(const std::string& _name);
	Brush(const std::string& _name, eMesh&&, std::vector<Texture*>);
	Brush(const std::string& _name, std::vector<Vertex> vertices,
				std::vector<unsigned  int> indices, std::vector<Texture*> textures);
	Brush(const std::string& _name, const ShapeData& data, uint32_t dimensions = 0);
	...
	virtual void								DrawInstanced(int32_t) override;
  ...
  virtual void												setTextures(std::vector<Texture*>);
	...
	const std::vector<std::vector<GLuint>>& GetIndicesLods() const {
		return indicesLods;
	}
...
{
public:
	static const int		MAXPARTICLES	= 1'000;
