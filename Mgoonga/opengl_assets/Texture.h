...
public:
	...
	bool TextureFromBuffer(const GLtype* buffer, GLuint Width, GLuint Height, GLuint format= GL_RGBA, GLenum wrap = GL_CLAMP_TO_EDGE, GLenum minFilter = GL_LINEAR)
	{
		...
		if constexpr (std::is_same_v<GLtype, GLfloat>)
			type = GL_FLOAT;
		else if constexpr (std::is_same_v<GLtype, GLubyte>)
			type = GL_UNSIGNED_BYTE;
		else
			static_assert(sizeof(GLtype) == 0, "Unsupported texture data type");

		if (format == GL_RED)
			internalFormat = (type == GL_FLOAT) ? GL_R32F : GL_R8;
		else if (format == GL_RGB)
			internalFormat = (type == GL_FLOAT) ? GL_RGB32F : GL_RGB8;
		else if (format == GL_RGBA)
			internalFormat = (type == GL_FLOAT) ? GL_RGBA32F : GL_RGBA8;
		else
			return false;
		...
		if (m_id == GetDefaultTextureId()) {
			_genTexture();
			newTexture = true; // Mark that storage needs to be allocated
		}
		...
		if (newTexture) {
			// Only allocate storage once
			glTexStorage2D(GL_TEXTURE_2D, 1, internalFormat, this->m_width, this->m_height);
		}

		// Update texture data
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_width, m_height, format, type, buffer);

		m_channels = (format == GL_RGBA) ? 4 : (format == GL_RGB) ? 3 : 1;

		// Set texture parameters (optional, but shouldn't hurt)
