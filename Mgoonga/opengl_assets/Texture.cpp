...
//-----------------------------------------
void Texture::SetUseMipMaps(bool _use_mip_maps)
{
	m_use_mipmaps = _use_mip_maps;
}
...
bool Texture::freeTexture(unsigned int _id)
{
	...
	if (_id != 0)
	{
		...
		{
			base::Log("free texture " + std::to_string(_id) + ": " + std::to_string(--textures_in_use));
		...
		{
			base::Log("trying to free texture which was not generated by texture class");
		}
	}
	return false;
}
...
bool Texture::loadTextureFromFile(const std::string& _path, GLenum format, GLenum wrap)
{
	...
	if (GetOpenGLContextThreadId() != std::this_thread::get_id())
	{
		base::Log(_path + "tried to load texture in non-OpenGL thread " + std::to_string(m_id));
		return false;
	}

	if(m_id != GetDefaultTextureId())
		base::Log(_path + "warning: trying to load already loaded texture " + std::to_string(m_id));

	uint32_t ilId;
	m_channels = eTextureImplDevIl::LoadTexture(m_path, ilId, m_width, m_height);
	if (m_channels == 0)
	{
		base::Log(_path + " was not loaded");
		return false;
	}
...
//-----------------------------------------------------------------------------------------------
void Texture::setNumRows(GLuint nrows)
{
	m_num_rows = nrows;
	if (m_num_rows > 1)
	{
...
void Texture::freeTexture()
{
	...
	if (m_id != 0)
	{
		...
		{
			base::Log("free texture " + std::to_string(m_id) + ": " + std::to_string(--textures_in_use));
...
bool Texture::makeDepthTexture()
{
	...
	//glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, m_width, m_width, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, NULL);
...
bool Texture::makeDepthTextureArray(int32_t _layers)
{
	...
	// Allocate storage (no initial data needed)
	glTexImage3D(GL_TEXTURE_2D_ARRAY,
		0,
		GL_DEPTH_COMPONENT32F,   // or DEPTH_COMPONENT24
	...
	// Filtering: choose based on sampling strategy
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, GL_NEAREST);   // for PCF -> linear, own pcf in shader -> nearest
	...
	const float border[4] = { 1.0f, 1.0f, 1.0f, 1.0f };   // depth=1 outside the map
	...
	// Comparison mode is set on the sampling pass, not required here:
...
bool Texture::loadHdr(const std::string& _path)
{
	...
	{
		base::Log("Failed to load HDR image.");
		return false;
	}
}
...
bool Texture::loadTexture2DArray(std::vector<std::string> _paths, GLenum _format)
{
	...
	for (int layer = 0; layer < _paths.size(); ++layer)
	{
		...
		{
			if(_format == GL_RGBA)
				glTexStorage3D(GL_TEXTURE_2D_ARRAY, 5, GL_RGBA8, m_width, m_height, layers); // after eTextureImplDevIl::LoadTexture assignes width an height
			else if(_format == GL_RED)
				glTexStorage3D(GL_TEXTURE_2D_ARRAY, 5, GL_R32F, m_width, m_height, layers);
		}
		uint8_t* data = nullptr;
		eTextureImplDevIl::AssignPixels(data, m_width, m_height);
		glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0/*mipmap_level*/, 0/*offset.x*/, 0/*offset.y*/, layer/*offset.z*/, m_width, m_height, 1 /*layer*/, _format, GL_UNSIGNED_BYTE, data);
		eTextureImplDevIl::DeleteImage(ilId);

		glGenerateMipmap(GL_TEXTURE_2D_ARRAY);

		glTextureParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		glTextureParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTextureParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTextureParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
void Texture::_genTexture()
{
	...
	if (m_id != GetDefaultTextureId())
	{
		base::Log({ "gen texture " + std::to_string(m_id) });
		++textures_in_use;
		indexes_in_use.insert(m_id);
	}
}
