...
ShapeData ShapeGenerator::makeMyPlane(float Width, float Height) {

	float widthN = 2 * tan(glm::radians(60.0f)) * 0.2; // perspective parameters
	...
	ShapeData::Vertex stackVerts[] = {		              //color                      //normals
		...
		glm::vec3(widthN / 2, -heightN / 2, 0),  glm::vec3(0, 0, 1),  glm::vec3(0, 0, 1),
		//
		glm::vec3(-widthF / 2, heightF / 2, -20), glm::vec3(0,1, 1),   glm::vec3(0, 0, 1),
		glm::vec3(widthF / 2, heightF / 2,  -20), glm::vec3(0, 1, 1), glm::vec3(0, 0, 1),
		glm::vec3(-widthF / 2,-heightF / 2, -20), glm::vec3(0, 1, 1),  glm::vec3(0, 0, 1),
		glm::vec3(widthF / 2, -heightF / 2, -20), glm::vec3(0, 1, 1),  glm::vec3(0, 0, 1),
		//line
		glm::vec3(0, 0, 0),  glm::vec3(1, 1, 1),  glm::vec3(0, 0, 1),
		glm::vec3(0, 0, -5), glm::vec3(1, 1, 1),  glm::vec3(0, 0, 1),
	};
...
ShapeData ShapeGenerator::makeSquare(float Width, float Height) {
	...
	ShapeData::Vertex stackVerts[] = {		              //color                      //normals
		...
		glm::vec3(-Width / 2,Height / 2, 0),  glm::vec3(0, 0, 1),  glm::vec3(0, 0, 1), //0
		glm::vec3(Width / 2, Height / 2, 0),  glm::vec3(0,0, 1),   glm::vec3(0, 0, 1), //1
		glm::vec3(Width / 2, -Height / 2,  0),  glm::vec3(0, 0, 1),  glm::vec3(0, 0, 1),
		glm::vec3(-Width / 2, -Height / 2, 0),  glm::vec3(0, 0, 1),  glm::vec3(0, 0, 1),  //3
...
//------------------------------------------------------------------------------------------------
ShapeData ShapeGenerator::makeCapsule(float radius, float height, int slices, int stacks)
{
	ShapeData data;

	const int rings = stacks * 2 + 1; // total vertical rings (bottom hemi + cylinder + top hemi)
	const int segments = slices;

	const float halfHeight = (height - 2.0f * radius) * 0.5f;

	const GLuint numVerts = (rings + 1) * (segments + 1);
	const GLuint numTris = rings * segments * 2;
	const GLuint numIndices = numTris * 3;

	data.vertices = new ShapeData::Vertex[numVerts];
	...
	for (int y = 0; y <= rings; ++y)
	{
		...
		if (y < stacks)
		{ // bottom hemisphere
			theta = glm::half_pi<float>() * float(y) / float(stacks);
			ringY = -halfHeight - radius * cos(theta);
			ringRadius = radius * sin(theta);
		}
		else if (y <= stacks)
		{ // cylinder equator row
			ringY = -halfHeight + (float(y - stacks) / float(stacks)) * (height - 2.0f * radius);
			ringRadius = radius;
		}
		else
		{ // top hemisphere
			theta = glm::half_pi<float>() * float(stacks - (y - stacks)) / float(stacks);
			ringY = halfHeight + radius * cos(theta);
			ringRadius = radius * sin(theta);
		}
		...
		for (int x = 0; x <= segments; ++x)
		{
			...
			glm::vec3 centerAxis(0.0f, glm::clamp(pos.y, -halfHeight, halfHeight), 0.0f);
			glm::vec3 normal = glm::normalize(pos - centerAxis);

			data.vertices[vertexIndex].position = pos;
			data.vertices[vertexIndex].normal = normal;
			data.vertices[vertexIndex].color = glm::vec3(1.0f, 1.0f, 1.0f); // white color
	...
	for (int y = 0; y < rings; ++y) {
		for (int x = 0; x < segments; ++x) {
			GLushort i0 = y * (segments + 1) + x;
			GLushort i1 = i0 + 1;
			GLushort i2 = i0 + (segments + 1);
			GLushort i3 = i2 + 1;

			// First triangle
			data.indices[index++] = i0;
			data.indices[index++] = i2;
			data.indices[index++] = i1;

			// Second triangle
			data.indices[index++] = i1;
			data.indices[index++] = i2;
			data.indices[index++] = i3;
		}
	}
