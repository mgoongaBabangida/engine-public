...
const size_t SPANISH_STARTING_POS_HEX_INDEX = 55;
const size_t PIRATE_STARTING_POS_HEX_INDEX = 26;
...
{
  m_visual_settings.m_texture_scales = { 10.0f, 7.0f , 4.0f };
  m_visual_settings.m_water_level = _pipeline.GetWaterHeight();
  m_visual_settings.m_ship_height_level = _pipeline.GetWaterHeight() - 0.05f;
}

//--------------------------------------------------------------------------------------------------------------------------------------
GameController::~GameController()
{
  if(m_network_settings.m_tcpAgent)
    dbb::NetWork::Shutdown();

  m_sound_settings.m_seagull_sound->Stop();
}
...
void GameController::Initialize()
{
  ...
  m_game->ShowFPS() = false;
  m_game->ShowMeasurementGrid() = false;
...
bool GameController::OnKeyPress(uint32_t _asci, KeyModifiers _modifier)
{
  ...
  if(m_game_logic.IsSpanishToMove() && m_focused_index < m_ships.size() && _GetSpanishShipByIndex(m_focused_index))
    focused_obj = _GetSpanishShipByIndex(m_focused_index);
  else if(m_game_logic.IsPirateToMove() && m_focused_index < m_ships.size() && _GetPirateShipByIndex(m_focused_index))
    focused_obj = _GetPirateShipByIndex(m_focused_index);
...
bool GameController::OnMouseRelease(KeyModifiers _modifier)
{
  if (m_right_button_pressed)
  {
    if (!m_current_path.empty())
    {
      ...
      m_game_gui.m_destination_text->content = "";
      m_visual_settings.m_path_mesh->UpdateData({}, {}, glm::vec4{ 1.0f });
    }
  ...
  if      (m_game_logic.IsSpanishToMove())
  {
    for (int i = 0; i < m_ships.size(); ++i)
    {
      ...
      if (_GetObject(m_ships[i]) == m_game->GetFocusedObject().get()
  ...
  else if (m_game_logic.IsPirateToMove())
  {
    for (int i = 0; i < m_ships_pirate.size(); ++i)
    {
      ...
      if (_GetObject(m_ships_pirate[i]) == m_game->GetFocusedObject().get()
...
void GameController::Update(float _tick)
{
  ...
  if (!m_right_button_pressed)
  {
    for (int i = 0; i < m_ships.size(); ++i)
    {
      ...
      if (shObject obj = m_ships[i]->GetScriptObject().lock())
      {
        ...
        {
          glm::vec3 destination = { m_current_path.front()->x(), m_visual_settings.m_ship_height_level, m_current_path.front()->z() };
          m_current_path.pop_front();
          m_ships[m_focused_index]->SetDestination(destination);
        }

        if (obj && obj.get() == m_game->GetFocusedObject().get())
          m_visual_settings.m_choice_circle->SetDots({ obj->GetTransform()->getTranslation() + glm::vec3{0.0f,0.06f,0.0f} }); //@todo
      }
    }
  ...
  if (m_game_logic.IsSpanishToMove())
  {
    if (std::all_of(m_has_moved.begin(), m_has_moved.end(), [](bool _val) { return _val; }))
    {
      ...
      m_game_logic.m_game_state = GameLogic::GameState::PIRATE_TO_MOVE;
    }
  }
  else if (m_game_logic.IsPirateToMove())
  {
    if (std::all_of(m_has_moved_pirate.begin(), m_has_moved_pirate.end(), [](bool _val) { return _val; }))
    {
      ...
      m_game_logic.m_game_state = GameLogic::GameState::SPANISH_TO_MOVE;
    }
  }

  //update icons move status
  for (int i = 0; i < m_game_gui.m_status_icons.size(); ++i)
  {
    const Texture* red_tex = m_game->GetTexture("red_circle");
    const Texture* green_tex = m_game->GetTexture("green_circle");
    if (m_has_moved[i])
      m_game_gui.m_status_icons[i]->SetTexture(*red_tex, { 0,0 }, { red_tex->m_width, red_tex->m_height });
    else
      m_game_gui.m_status_icons[i]->SetTexture(*green_tex, { 0,0 }, { green_tex->m_width, green_tex->m_height });
  }

  //update icons move status pirate
  for (int i = 0; i < m_game_gui.m_status_icons_pirate.size(); ++i)
  {
    const Texture* red_tex = m_game->GetTexture("red_circle");
    const Texture* green_tex = m_game->GetTexture("green_circle");
    if (m_has_moved_pirate[i])
      m_game_gui.m_status_icons_pirate[i]->SetTexture(*red_tex, { 0,0 }, { red_tex->m_width, red_tex->m_height });
    else
      m_game_gui.m_status_icons_pirate[i]->SetTexture(*green_tex, { 0,0 }, { green_tex->m_width, green_tex->m_height });
  }
  ...
  trans.setTranslation({ m_hexes[99].x() + offset.x, m_visual_settings.m_water_level + 0.2f, m_hexes[99].z() + offset.y });
  trans.billboard(-m_camera.get().getDirection());
  m_game_gui.m_base_labels[0]->mvp = m_camera.get().getProjectionMatrix() * m_camera.get().getWorldToViewMatrix() * trans.getModelMatrix();

  trans.setTranslation({ m_hexes[38].x() + offset.x, m_visual_settings.m_water_level + 0.2f, m_hexes[38].z() + offset.y });
  trans.billboard(-m_camera.get().getDirection());
  m_game_gui.m_base_labels[1]->mvp = m_camera.get().getProjectionMatrix() * m_camera.get().getWorldToViewMatrix() * trans.getModelMatrix();

  trans.setTranslation({ m_hexes[83].x() + offset.x, m_visual_settings.m_water_level + 0.2f, m_hexes[83].z() + offset.y });
  trans.billboard(-m_camera.get().getDirection());
  m_game_gui.m_base_labels[2]->mvp = m_camera.get().getProjectionMatrix() * m_camera.get().getWorldToViewMatrix() * trans.getModelMatrix();

  trans.setTranslation({ m_hexes[141].x() + offset.x, m_visual_settings.m_water_level + 0.2f, m_hexes[141].z() + offset.y });
  trans.billboard(-m_camera.get().getDirection());
  m_game_gui.m_base_labels[3]->mvp = m_camera.get().getProjectionMatrix() * m_camera.get().getWorldToViewMatrix() * trans.getModelMatrix();
  ...
  if (!m_dice_rolled)
  {
    if (m_game_logic.IsSpanishToMove())
    {
      ...
      for (unsigned int i = 0; i < m_ships.size(); ++i)
      {
        if (m_ships[i] == nullptr)
          continue;
        if (_GetObject(m_ships[i]) == _picked.get())
        {
          index_picked = i;
          m_game_gui.m_ship_icons[index_picked]->SetRenderingFunc(GUI::RenderFunc::Default);
          m_focused_index = index_picked;
        }
        else
          m_game_gui.m_ship_icons[i]->SetRenderingFunc(GUI::RenderFunc::GreyKernel);
      }
      if(index_picked != -1)
    ...
    else if (m_game_logic.IsPirateToMove())
    {
      ...
      for (unsigned int i = 0; i < m_ships_pirate.size(); ++i)
      {
        if (m_ships_pirate[i] == nullptr)
          continue;
        if (_GetObject(m_ships_pirate[i]) == _picked.get())
        {
          index_picked = i;
          m_game_gui.m_ship_icons_pirate[index_picked]->SetRenderingFunc(GUI::RenderFunc::Default);
          m_focused_index = index_picked;
        }
        else
          m_game_gui.m_ship_icons_pirate[i]->SetRenderingFunc(GUI::RenderFunc::GreyKernel);
      }
      if (index_picked != -1)
        m_game->SetFocused(_picked);
    }
...
void GameController::OnObjectPickedWithRight(std::shared_ptr<eObject> _picked)
{
  if (_picked)
  {
    if (     size_t index = _GetPirateShipIndex(_picked); m_game_logic.IsSpanishToMove() && index < m_ships_pirate.size())
    {
      for (int i = 0; i < m_ships.size(); ++i)
      {
        if (m_ships[i] == nullptr)
    ...
    else if (size_t index = _GetSpanishShipIndex(_picked); m_game_logic.IsPirateToMove() && index < m_ships.size())
    {
      for (int i = 0; i < m_ships_pirate.size(); ++i)
      {
        if (_GetObject(m_ships_pirate[i]) == m_game->GetFocusedObject().get()
          && m_has_moved_pirate[i] == true)
        {
          ...
          if (dbb::round(distance, 2) <= Hex::radius * 1.0f) // one hex
          {
            ...
            OnShoot(_GetObject(m_ships_pirate[i]));
...
void GameController::OnFrameMoved(std::shared_ptr<GUI> _frame)
{
  ...
  for (unsigned int i = 0; i < m_game_gui.m_ship_icons.size(); ++i)
  {
    if (m_game_gui.m_ship_icons[i]->IsHover())
    {
      if (m_has_gold_index == -1)
        m_has_gold_index = i;
      _frame->Move({ m_game_gui.m_ship_icons[m_has_gold_index]->GetTopLeft().x - offset.x, m_game_gui.m_ship_icons[m_has_gold_index]->GetTopLeft().y - offset.y });
      return;
    }
  }
  if (m_has_gold_index != -1)
  ...
  if (auto hovered = m_game->GetHoveredObject(); hovered)
  {
    for (unsigned int i = 0; i < m_ships.size(); ++i)
    {
      if (hovered->GetScript() == m_ships[i] && m_ships[i] != nullptr)
      {
        m_has_gold_index = i;
        glm::vec2 offset{ 10,7 };
        _frame->Move({ m_game_gui.m_ship_icons[i]->GetTopLeft().x - offset.x, m_game_gui.m_ship_icons[i]->GetTopLeft().y - offset.y });
        return;
      }
    }
  }
...
{
  glm::vec4 modelCenterShooter = _shooter->GetTransform()->getModelMatrix() * glm::vec4(_shooter->GetCollider()->GetCenterLocalSpace(), 1.0f);
  glm::vec4 modelCenterTarget = m_target->GetTransform()->getModelMatrix() * glm::vec4(m_target->GetCollider()->GetCenterLocalSpace(), 1.0f);
...
void GameController::_OnTCPMessageRecieved(const std::vector<uint32_t> _content)
{
  ...
  if(_content[0] == (uint32_t)NetworkSettings::MessageType::MOVE)
  {
    glm::vec3 destination = { m_current_path.front()->x(), m_visual_settings.m_ship_height_level, m_current_path.front()->z() };
    ...
    if (m_game_logic.IsSpanishToMove() && m_game_logic.IsPirateSide())
      m_ships[m_focused_index]->SetDestination(destination);
    else if (m_game_logic.IsPirateToMove() && m_game_logic.IsSpanishSide())
      m_ships_pirate[m_focused_index]->SetDestination(destination);
    else
    {
      m_current_path.clear();
    ...
    auto cur_hex = std::find_if(m_hexes.begin(), m_hexes.end(), [this](const Hex& hex)
      {
        return glm::length(m_game->GetFocusedObject()->GetTransform()->getTranslation() - glm::vec3{ hex.x(), m_visual_settings.m_water_level, hex.z() }) < (0.5f * 0.57f); //radius
      });
    cur_hex->SetTaken(false);
    _UpdateWarrning("");
  }
...
void GameController::_InitializeDiceLogicAndVisual()
{
  ...
  m_game_gui.m_dice_gui = std::make_shared<GUI>((int)pos_x, (int)pos_y, (int)icon_size.x, (int)icon_size.y, m_game->Width(), m_game->Height());
  auto dice_tex = _GetDiceTexture();
  m_game_gui.m_dice_gui->SetTexture(*dice_tex, { 0,0 }, { dice_tex->m_width, dice_tex->m_height });
  m_game_gui.m_dice_gui->SetTakeMouseEvents(true);
  m_game_gui.m_dice_gui->SetCommand(std::make_shared<GUICommand>([this]()
    {
  ...
  m_game->AddGUI(m_game_gui.m_dice_gui);
  m_game->AddInputObserver(m_game_gui.m_dice_gui.get(), MONOPOLY);
}
...
void GameController::_InitializeShipIcons()
{
  ...
  for (int i = 0; i < m_game_logic.m_ship_quantity; ++i)
  {
    float pos_x = (10.0f + icon_size.x) * i;
    float pos_y = 10.0f;
    const Texture* ship_tex = m_game->GetTexture("ship1");
    std::shared_ptr<GUI> ship_gui = std::make_shared<GUI>((int)pos_x, (int)pos_y, (int)icon_size.x, (int)icon_size.y, 
    ...
    ship_gui->SetTakeMouseEvents(true);
    ship_gui->SetCommand(std::make_shared<GUICommand>([this, ship_gui]()
    {
        for (int i = 0; i < m_game_gui.m_ship_icons.size(); i++)
        {
          if (m_game_gui.m_ship_icons[i]->IsHover())
          {
            if (!m_dice_rolled && m_game_logic.IsSpanishToMove())
            {
              m_game->SetFocused(m_ships[i]->GetScriptObject().lock());
              m_game_gui.m_ship_icons[i]->SetRenderingFunc(GUI::RenderFunc::Default);
              m_focused_index = i;
            }
          }
          else
    ...
    m_game_gui.m_ship_icons.push_back(ship_gui);
  }

  //pirate ship icons
  for (int i = 0; i < m_game_logic.m_ship_quantity_pirate; ++i)
  {
    ...
    ship_gui->SetCommand(std::make_shared<GUICommand>([this, ship_gui]()
      {
        for (int i = 0; i < m_game_gui.m_ship_icons_pirate.size(); i++)
        {
          if (m_game_gui.m_ship_icons_pirate[i]->IsHover())
          {
            if (!m_dice_rolled && m_game_logic.IsPirateToMove())
            {
              m_game->SetFocused(m_ships_pirate[i]->GetScriptObject().lock());
              m_game_gui.m_ship_icons_pirate[i]->SetRenderingFunc(GUI::RenderFunc::Default);
              m_focused_index = i;
            }
          }
          else
    ...
    m_game->AddGUI(ship_gui);
    m_game->AddInputObserver(ship_gui.get(), MONOPOLY);
    m_game_gui.m_ship_icons_pirate.push_back(ship_gui);
  }
  ...
  for (int i = 0; i < m_game_logic.m_ship_quantity; ++i)
  {
    ...
    m_game_gui.m_status_icons.push_back(status_gui);
  }
  ...
  for (int i = 0; i < m_game_logic.m_ship_quantity_pirate; ++i)
  {
    ...
    status_gui->SetTransparent(true);
    m_game->AddGUI(status_gui);
    m_game_gui.m_status_icons_pirate.push_back(status_gui);
  }
}
...
//--------------------------------------------------------------------------------------------------------------------------------------
void GameController::_InitializeTerrain()
{
  //TERRAIN
  std::unique_ptr<TerrainModel> terrainModel = m_modelManager->CloneTerrain("simple");
  Texture blue = Texture::GetTexture1x1(BLUE);
  terrainModel->Initialize(const_cast<Texture*>(m_texManager->Find("Tgrass0_d")),
                          const_cast<Texture*>(m_texManager->Find("Tgrass0_d")),
                          &blue,
                          const_cast<Texture*>(m_texManager->Find("TOcean0_s")),
                          true,
                          m_visual_settings.m_height_scale,
                          m_visual_settings.m_max_height, 0, 1, 1);
  ...
  terrain_types.insert({ "mounten",		0.0f, m_visual_settings.m_texturing_water_level, {0.5f, 0.5f, 0.0f } });
  terrain_types.insert({ "grass",		m_visual_settings.m_texturing_water_level,	1.0f, {0.0f, 1.0f, 0.0f} });
  ...
  for (const auto& type : terrain_types)
  {
    ...
    m_pipeline.get().SetUniformData("class ePhongRender",
      ...
      m_visual_settings.m_texture_scales[counter]);
    ++counter;
  }
  m_pipeline.get().SetUniformData("class ePhongRender",
    "base_start_heights[" + std::to_string(counter) + "]",
...
void GameController::_InitializeShips()
{
  ...
  material.emission_strength = 0.f;
  ...
  const size_t distance_in_hex = 40;
  for (int i = 0; i < m_game_logic.m_ship_quantity; ++i)
  {
    ...
    ship->GetTransform()->setTranslation(glm::vec3(m_hexes[SPANISH_STARTING_POS_HEX_INDEX + i* distance_in_hex].x(), m_visual_settings.m_ship_height_level, m_hexes[SPANISH_STARTING_POS_HEX_INDEX + i * distance_in_hex].z()));
    ship->GetTransform()->setUp(glm::vec3(0.0f, 0.0f, 1.0f));
    ship->GetTransform()->setForward(glm::vec3(-1.0f, 0.0f, 0.0f));
    ship->GetTransform()->turnTo(glm::vec3{ m_hexes[110].x(), m_visual_settings.m_ship_height_level, m_hexes[110].z() }, 0.05f);

    m_hexes[SPANISH_STARTING_POS_HEX_INDEX + i * distance_in_hex].SetTaken(true);
    ...
    auto script = new eShipScript(m_game,
                                  ...
                                  m_texManager->Find("Tatlas2"),
                                  m_soundManager->GetSound("shot_sound"),
                                  m_visual_settings.m_water_level);
  ...
  for (int i = 0; i < m_game_logic.m_ship_quantity_pirate; ++i)
  {
    ...
    //shObject ship = factory.CreateObject(m_modelManager->Find("wall_cube"), eObject::RenderType::PHONG, "Pirate" + std::to_string(i));
    ship->GetTransform()->setScale(glm::vec3(0.04f, 0.04f, 0.04f));
    ship->GetTransform()->setTranslation(glm::vec3(m_hexes[PIRATE_STARTING_POS_HEX_INDEX + i * distance_in_hex].x(), m_visual_settings.m_ship_height_level, m_hexes[PIRATE_STARTING_POS_HEX_INDEX + i * distance_in_hex].z()));
    ship->GetTransform()->setUp(glm::vec3(0.0f, 0.0f, 1.0f));
    ship->GetTransform()->setRotation(0.0f, PI/2, 0.0f);
    ship->GetTransform()->setForward(glm::vec3(0.0f, 0.0f, -1.0f));

    ship->SetInstancingTag("pirate_ship");
    
    m_hexes[PIRATE_STARTING_POS_HEX_INDEX + i * distance_in_hex].SetTaken(true);

    if(false) // if(i == 0)  // @todo correct loading and sharing of the textures like in reload function
    {
      for (auto& mesh : ship->GetModel()->Get3DMeshes())
      {
        Texture t;
        if (auto textures = mesh->GetTextures(); !textures.empty())
        {
          if (textures[0].m_path.find("barrel_texture1k") != std::string::npos)
          {
            ...
            mesh->GetMaterial()->used_textures.insert(Material::TextureType::METALLIC);
            ...
            const_cast<I3DMesh*>(mesh)->AddTexture(&t);
            mesh->GetMaterial()->used_textures.insert(Material::TextureType::NORMAL);
          }
...
void GameController::_InitializeBases()
{
  ...
  base1->GetTransform()->setTranslation(glm::vec3(m_hexes[99].x(), m_visual_settings.m_water_level + 0.15f, m_hexes[99].z()));
  ...
  m_game_gui.m_base_labels.push_back(std::make_shared<Text>());
  m_game_gui.m_base_labels[0]->content = "Veracruz";
  m_game_gui.m_base_labels[0]->font = "edwardian";
  m_game_gui.m_base_labels[0]->pos_x = 0.0f;
  m_game_gui.m_base_labels[0]->pos_y = 0.0f;
  m_game_gui.m_base_labels[0]->scale = 1.0f;
  m_game_gui.m_base_labels[0]->color = glm::vec3(1.0f, 1.0f, 1.0f);
  m_game->AddText(m_game_gui.m_base_labels[0]);
  ...
  base2->GetTransform()->setTranslation(glm::vec3(m_hexes[38].x(), m_visual_settings.m_water_level + 0.15f, m_hexes[38].z()));
  ...
  m_game_gui.m_base_labels.push_back(std::make_shared<Text>());
  m_game_gui.m_base_labels[1]->content = "Cartagena";
  m_game_gui.m_base_labels[1]->font = "edwardian";
  m_game_gui.m_base_labels[1]->pos_x = 0.0f;
  m_game_gui.m_base_labels[1]->pos_y = 0.0f;
  m_game_gui.m_base_labels[1]->scale = 1.0f;
  m_game_gui.m_base_labels[1]->color = glm::vec3(1.0f, 1.0f, 1.0f);
  m_game->AddText(m_game_gui.m_base_labels[1]);
  ...
  base3->GetTransform()->setTranslation(glm::vec3(m_hexes[83].x(), m_visual_settings.m_water_level + 0.15f, m_hexes[83].z()));
  ...
  m_game_gui.m_base_labels.push_back(std::make_shared<Text>());
  m_game_gui.m_base_labels[2]->content = "Casablanca";
  m_game_gui.m_base_labels[2]->font = "edwardian";
  m_game_gui.m_base_labels[2]->pos_x = 0.0f;
  m_game_gui.m_base_labels[2]->pos_y = 0.0f;
  m_game_gui.m_base_labels[2]->scale = 1.0f;
  m_game_gui.m_base_labels[2]->color = glm::vec3(1.0f, 1.0f, 1.0f);
  m_game->AddText(m_game_gui.m_base_labels[2]);
  ...
  base4->GetTransform()->setTranslation(glm::vec3(m_hexes[141].x(), m_visual_settings.m_water_level + 0.15f, m_hexes[141].z()));
  ...
  m_game_gui.m_base_labels.push_back(std::make_shared<Text>());
  m_game_gui.m_base_labels[3]->content = "Sevillia";
  m_game_gui.m_base_labels[3]->font = "edwardian";
  m_game_gui.m_base_labels[3]->pos_x = 0.0f;
  m_game_gui.m_base_labels[3]->pos_y = 0.0f;
  m_game_gui.m_base_labels[3]->scale = 1.0f;
  m_game_gui.m_base_labels[3]->color = glm::vec3(1.0f, 1.0f, 1.0f);
  m_game->AddText(m_game_gui.m_base_labels[3]);
}
...
//--------------------------------------------------------------------------------------------------------------------------------------
void GameController::_InitializeGLData()
{
  ObjectFactoryBase factory;
  ...
  m_visual_settings.m_choice_circle = new SimpleGeometryMesh({ NONE }, 0.15f, SimpleGeometryMesh::GeometryType::Circle, { m_visual_settings.m_hdr_strangth, m_visual_settings.m_hdr_strangth, 0.0f });
  m_game->AddObject(factory.CreateObject(std::make_shared<SimpleModel>(m_visual_settings.m_choice_circle), eObject::RenderType::GEOMETRY, "Choice circle"));
  m_visual_settings.m_path_mesh = new LineMesh({}, {}, glm::vec4{ 1.0f,1.0f,1.0f,1.0f });
  m_game->AddObject(factory.CreateObject(std::make_shared<SimpleModel>(m_visual_settings.m_path_mesh), eObject::RenderType::LINES, "Path mesh"));

  m_game_gui.m_destination_text = std::make_shared<Text>();
  m_game_gui.m_destination_text->content = std::to_string(m_current_path.size());
  m_game_gui.m_destination_text->font = "edwardian";
  m_game_gui.m_destination_text->pos_x = 0.0f;
  m_game_gui.m_destination_text->pos_y = 0.0f;
  m_game_gui.m_destination_text->scale = 1.0f;
  m_game_gui.m_destination_text->color = glm::vec3(1.0f, 1.0f, 1.0f);
  m_game_gui.m_destination_text->mvp = m_camera.get().getProjectionMatrix() * m_camera.get().getWorldToViewMatrix() * UNIT_MATRIX;
  m_game->AddText(m_game_gui.m_destination_text);

  m_game_gui.m_warning = std::make_shared<Text>();
  m_game_gui.m_warning->font = "ARIALN";
  m_game_gui.m_warning->pos_x = 200.0f;
  m_game_gui.m_warning->pos_y = 25.0f;
  m_game_gui.m_warning->scale = 1.0f;
  m_game_gui.m_warning->color = glm::vec3(1.0f, 1.0f, 1.0f);
  m_game_gui.m_warning->mvp = glm::ortho(0.0f, (float)m_game->Width(), 0.0f, (float)m_game->Height());
  m_game->AddText(m_game_gui.m_warning);
}
...
void GameController::_InstallTcpServer()
{
  if (!m_network_settings.m_tcpAgent)
  {
    ...
    {
      m_network_settings.m_tcpAgent = std::make_unique<Server>();
      m_network_settings.m_tcpAgent->ConnectionEstablished.Subscribe([this](const dbb::TCPConnection& _connection) {_OnConnectionEstablished(_connection); });
      if (m_network_settings.m_tcpAgent->Initialize(dbb::IPEndPoint{ "0.0.0.0", 8080 }))//134.238.94.205 //208.67.222.222
      {
        m_network_settings.m_tcpTimer.reset(new math::Timer([this]()->bool
          {
            if (m_network_settings.m_tcpAgent->IsConnected())
              m_network_settings.m_tcpAgent->Frame();
            return true;
          }));
        m_network_settings.m_tcpTimer->start(15); //~70 fps
      }
    }
  }
}
...
void GameController::_InstallTcpClient()
{
  if (!m_network_settings.m_tcpAgent)
  {
    if (dbb::NetWork::Initialize())
    {
      m_network_settings.m_tcpAgent = std::make_unique<Client>();
      m_network_settings.m_tcpAgent->ConnectionEstablished.Subscribe([this](const dbb::TCPConnection _connection) {_OnConnectionEstablished(_connection); });
      if (m_network_settings.m_tcpAgent->Initialize(dbb::IPEndPoint{ "127.0.0.1", 8080 })) //"109.95.50.27 // "192.168.2.102 //134.238.94.205 /208.67.222.222
      {
        m_network_settings.m_tcpTimer.reset(new math::Timer([this]()->bool
          {
            if (m_network_settings.m_tcpAgent->IsConnected())
...
void GameController::_UpdatePathVisual()
{
  if (m_current_path.empty())
  {
    m_visual_settings.m_path_mesh->UpdateData({}, {}, { 1.0f, 1.0f ,1.0f, 1.0f });
    return;
  }
  ...
  glm::vec3 pos;
  if(m_game_logic.IsSpanishToMove())
    pos = _GetSpanishShipByIndex(m_focused_index)->GetTransform()->getTranslation();
  else if(m_game_logic.IsPirateToMove())
    pos = _GetPirateShipByIndex(m_focused_index)->GetTransform()->getTranslation();
  ...
  m_visual_settings.m_path_mesh->UpdateData(verices, indices, { 1.0f, 1.0f ,1.0f, 1.0f });
...
{
  m_game_gui.m_warning->content = _message;
}

//--------------------------------------------------------------------------------------------------------------------------------------
void GameController::_InitializeSounds()
{
  m_sound_settings.m_seagull_sound = m_soundManager->GetSound("seagull_sound");
  m_sound_settings.m_seagull_sound->Play();
}

//--------------------------------------------------------------------------------------------------------------------------------------
void GameController::_InitializeDebug()
{
  //DEBUG FIELDS
  ...
  std::function<void()> switch_rendering_mode_callback = [this]()
  {
    if (TerrainModel* terrrain_model = dynamic_cast<TerrainModel*>(m_terrain->GetModel()); terrrain_model)
    {
      if (terrrain_model->getMeshes()[0]->GetRenderMode() == Brush::RenderMode::DEFAULT)
        terrrain_model->getMeshes()[0]->SetRenderMode(Brush::RenderMode::WIREFRAME);
      else
        terrrain_model->getMeshes()[0]->SetRenderMode(Brush::RenderMode::DEFAULT);
    }
  };
  ...
  {
    if (m_network_settings.m_tcpAgent)
      m_network_settings.m_tcpAgent->SendMsg(std::vector<uint32_t>{ 3, 1, 2, 3 });
  };
  ...
  m_debug_window->Add(SLIDER_FLOAT, "Texture Scale under watter", &m_visual_settings.m_texture_scales[0]);
  m_debug_window->Add(SLIDER_FLOAT, "Texture Scale grass", &m_visual_settings.m_texture_scales[1]);
  m_debug_window->Add(SLIDER_FLOAT, "Texture Scale grass 2", &m_visual_settings.m_texture_scales[2]);
  m_debug_window->Add(BUTTON, "Debug Hexes", (void*)&debug_hexes);
  m_debug_window->Add(SLIDER_FLOAT, "Height Scale", &m_visual_settings.m_height_scale);
  m_debug_window->Add(SLIDER_FLOAT, "Max Height", &m_visual_settings.m_max_height);
  m_debug_window->Add(SLIDER_FLOAT, "Water Level Height", &m_visual_settings.m_water_level);
  ...
  m_debug_window->Add(SLIDER_FLOAT, "HDR Strength", (void*)&m_visual_settings.m_hdr_strangth);
...
void GameController::_SendMoveMsg()
{
  ...
  if (!m_game_logic.IsBothSide()) // send tcp event
  {
    std::vector<uint32_t> content{ (uint32_t)(m_current_path.size() + 2),
                                   (uint32_t)NetworkSettings::MessageType::MOVE,
                                   m_focused_index };
    std::vector<uint32_t> path(_GetCurPathIndices());
    content.insert(content.end(), path.begin(), path.end());

    if (m_network_settings.m_tcpAgent)
      m_network_settings.m_tcpAgent->SendMsg(std::move(content));
  }
}

//--------------------------------------------------------------------------------------------------------------------------------------
void GameController::_UpdateDebug()
{
  //update debug
  for (int i = 0; i < m_visual_settings.m_texture_scales.size(); ++i)
  {
    m_pipeline.get().SetUniformData("class ePhongRender",
      "textureScale[" + std::to_string(i) + "]",
      m_visual_settings.m_texture_scales[i]);
  }

  m_visual_settings.m_choice_circle->SetColor({ m_visual_settings.m_hdr_strangth, m_visual_settings.m_hdr_strangth, 0.f, 1.f });

  //update texts debug
  for (int i = 0; i < m_visual_settings.texts.size(); ++i)
  {
    Transform trans;
    ...
    trans.setRotation(-PI / 2, -PI / 2, 0.0f);
    m_visual_settings.texts[i]->mvp = m_camera.get().getProjectionMatrix() * m_camera.get().getWorldToViewMatrix() * trans.getModelMatrix();
  }

  // update heights debug
  static float height_scale = m_visual_settings.m_height_scale;
  static float max_height = m_visual_settings.m_max_height;
  if (height_scale != m_visual_settings.m_height_scale || max_height != m_visual_settings.m_max_height)
  {
    if (auto* terrainModel = dynamic_cast<TerrainModel*>(m_terrain->GetModel()))
    {
      ...
      terrainModel->Initialize(const_cast<Texture*>(m_texManager->Find("Tgrass0_d")),
                               ...
                               true,
                               m_visual_settings.m_height_scale,
                               m_visual_settings.m_max_height);
      height_scale = m_visual_settings.m_height_scale;
      max_height = m_visual_settings.m_max_height;
    }
  }
  static float water_level = m_visual_settings.m_texturing_water_level;
  if (water_level != m_visual_settings.m_texturing_water_level)
  {
    std::set<TerrainType> terrain_types;
    terrain_types.insert({ "mounten",		0.0f, m_visual_settings.m_texturing_water_level, {0.5f, 0.5f, 0.0f } });
    terrain_types.insert({ "grass",		m_visual_settings.m_texturing_water_level,	1.0f, {0.0f, 1.0f, 0.0f} });
    ...
    m_pipeline.get().SetUniformData("class ePhongRender",
      "base_start_heights[" + std::to_string(counter) + "]",
      1.0f);
    water_level = m_visual_settings.m_texturing_water_level;
  }
}
...
void GameController::_UpdateTextPath()
{
  if (!m_current_path.empty())
  {
    if (Hex* dest = m_current_path.back(); dest)
    {
      ...
      trans.setRotation(-PI / 2, -PI / 2, 0.0f);
      m_game_gui.m_destination_text->mvp = m_camera.get().getProjectionMatrix() * m_camera.get().getWorldToViewMatrix() * trans.getModelMatrix();
    }
  }
  else
  {
...
eShipScript* GameController::_GetShipScript(const eObject* _ship) const
{
  for (int i = 0; i < m_ships.size(); ++i)
  {
    if (_GetObject(m_ships[i]) == _ship)
      return m_ships[i];
  }
  for (int i = 0; i < m_ships_pirate.size(); ++i)
  {
    if (_GetObject(m_ships_pirate[i]) == _ship)
...
//--------------------------------------------------------------------------------------------------------------------------------------
void   GameController::_UpdateCurrentPath(int32_t _x, int32_t _y)
{
  if (m_has_gold_index == -1)
  {
  ...
  else // player rolled dice and has not moved yet. so, lets calculate wheather and how they can move
  {
    ...
    dbb::plane pl(glm::vec3(1.0f, m_visual_settings.m_water_level, 1.0f),
                  glm::vec3(0.0f, m_visual_settings.m_water_level, 0.0f),
                  glm::vec3(0.0f, m_visual_settings.m_water_level, 1.0f)); // arbitrary triangle on waterHeight plane

    glm::vec3 target = dbb::intersection(pl, m_camera.get().getCameraRay().GetLine(_x, _y));
    for (auto& hex : m_hexes)
    {
      if (hex.IsOn(target.x, target.z) && (hex.IsWater(m_terrain, m_visual_settings.m_water_level) || hex.IsBase()))
      {
        if (hex.IsTaken())
        {
        ...
        if (auto cur_hex = _GetCurHex(); cur_hex)
        {
          m_current_path = cur_hex->MakePath(&hex, 
                                             m_terrain, 
                                             m_visual_settings.m_water_level, 
                                             m_game_logic.m_game_state == GameLogic::GameState::SPANISH_TO_MOVE);
...
//--------------------------------------------------------------------------------------------------------------------------------------
std::shared_ptr<eObject> GameController::_GetPirateShipByIndex(size_t _index)
{
  return m_ships_pirate[_index]->GetScriptObject().lock();
}

//--------------------------------------------------------------------------------------------------------------------------------------
std::shared_ptr<eObject> GameController::_GetSpanishShipByIndex(size_t _index)
{
  return m_ships[_index]->GetScriptObject().lock();
}

//--------------------------------------------------------------------------------------------------------------------------------------
eObject* GameController::_GetObject(eShipScript* _script) const
{
  if (auto obj = _script->GetScriptObject().lock(); obj)
    return _script->GetScriptObject().lock().get();
  else
    return nullptr;
}
...
void GameController::_DebugHexes()
{
  ...
  for (auto& hex : m_hexes)
  {
    ...
    m_visual_settings.texts.push_back(text);
