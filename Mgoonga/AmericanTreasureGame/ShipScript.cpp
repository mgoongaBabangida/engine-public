...
, m_flag(new eObject{})
{
	...
	move_speed = 1.0f / 30.0f;
	flag_scale = { 0.02f, 0.02f ,0.02f };

	m_flag->SetTransform(new Transform);
	std::shared_ptr<Brush> mesh(new Brush("flag"));
...
void eShipScript::Update(float _tick)
{
	...
	if (shObject object = m_object.lock(); object)
	{
		if (object && destination != NONE)
		{
			...
			{
				if (glm::length2(object->GetTransform()->getTranslation() - destination) > move_speed/5) //@todo _tick
				{
					glm::vec3 translation = object->GetTransform()->getTranslation();
					glm::mat4 modelMatrix = object->GetTransform()->getModelMatrix();
					object->GetTransform()->setTranslation(translation + glm::mat3(modelMatrix) * glm::vec3(object->GetTransform()->getForward() * move_speed));
				}
				else
		...
		{
			m_state = HIT;
			m_drowning_animation = std::make_unique<math::AnimationLinear<float>>(std::vector<float>{0}, std::vector<float>{PI / 4}, 5'000);
		...
		{
			m_state = DROWNING;
			m_drowning_animation = std::make_unique<math::AnimationLinear<float>>(std::vector<float>{ object->GetTransform()->getTranslation().y, },
