...
namespace dbb
{
  ...
  DLL_MATH bool ReadFromFile(std::vector<float>& _data, const std::string& _filename);
	...
	DLL_MATH std::string ExtractFilename(const std::string& _path);

	DLL_MATH float MapValueLinear(float x, float inputMin, float inputMax, float outputMin, float outputMax);

	DLL_MATH float computeDepthBufferValue(float z_eye, float fov_deg, float aspect, float near, float far);
}
