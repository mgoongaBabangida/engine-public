...
public:
	...
	void				    setRotation(glm::quat q)	 { q_rotation = glm::normalize(q);	UpdateModelMatrix(); }
	...
	static float			AngleDegreesBetweenVectors(glm::vec3 start, glm::vec3 dest);
  ...
  glm::vec3            getScaleAsVector() const { return m_scale; }
	...
	bool				turnToOld(glm::vec3 dest, float speed = 0.f);
	bool				turnTo(glm::vec3 dest, float speed = 0.f);
