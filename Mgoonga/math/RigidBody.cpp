...
namespace dbb
{
  ...
  void RigidBody::Update(float _deltaTime) //needs to be called at fixed fps
  {
    ...
    if(HasVolume())
    {
      ...
      if (glm::length2(m_angVel) < 0.0001f)
        m_angVel = glm::vec3(0.0f);
  ...
  void RigidBody::SetCollider(ICollider* _c)
  {
    if (_c)
    {
      //@todo check if works correctly, base get center or other? Need proper GetCenter !
      m_collider = _c;
      m_position = m_collider->GetCenter();
      m_orientation = m_collider->GetOrientation();
    }
  ...
  {
    return m_mass == 0 && m_gravity_applicable == false;
  }
