...
namespace dbb
{
  ...
  //---------------------------------------------------------------------------------------------------
  void ProjectTriangle(const dbb::triangle& tri, const glm::vec3& axis, float& outMin, float& outMax)
  {
    outMin = outMax = glm::dot(axis, glm::vec3(tri[0][0], tri[0][1], tri[0][2]));
    for (int i = 1; i < 3; ++i)
    {
      float projection = glm::dot(axis, glm::vec3(tri[i][0], tri[i][1], tri[i][2]));
  ...
  //----------------------------------------------------------------------------------------
  void ProjectOBB(const OBB& obb, const glm::vec3& axis, float& outMin, float& outMax)
  {
    glm::vec3 center = obb.origin;
    ...
    float r = 0.0f;
    for (int i = 0; i < 3; ++i)
      r += obb.size[i] * std::abs(glm::dot(axis, u[i])); // obb half size

    float c = glm::dot(axis, center);
    outMin = c - r;
    outMax = c + r;
  }

  //-------------------------------------------------------------
  CollisionManifold TriangleOBB(const dbb::triangle& _triangle, const OBB& _obb)
  {
    CollisionManifold result;
    result.colliding = false;

    glm::vec3 triangle_p1(_triangle[0][0], _triangle[0][1], _triangle[0][2]);
    glm::vec3 triangle_p2(_triangle[1][0], _triangle[1][1], _triangle[1][2]);
    glm::vec3 triangle_p3(_triangle[2][0], _triangle[2][1], _triangle[2][2]);
    ...
    glm::vec3 u0(o[0], o[1], o[2]);
    glm::vec3 u1(o[3], o[4], o[5]);
    glm::vec3 u2(o[6], o[7], o[8]);
    ...
    for (int i = 0; i < 13; ++i)
    {
      glm::vec3 axis = test[i];
      result.info.axis[i] = axis;
      ...
      float minTri, maxTri;
      ProjectTriangle(_triangle, axis, minTri, maxTri);

      float minOBB, maxOBB;
      ProjectOBB(_obb, axis, minOBB, maxOBB);

      float overlap = std::min(maxTri, maxOBB) - std::max(minTri, minOBB);
      ...
      if (overlap < minPenetration)
      {
        ...
        float centerTri = (minTri + maxTri) * 0.5f;
        float centerOBB = (minOBB + maxOBB) * 0.5f;
        if (centerTri < centerOBB)
          bestAxis = -bestAxis;
      }
    }
    ...
    // Multi-contact: collect triangle vertices inside OBB slab
    result.contacts.clear();
    float planeMin = glm::dot(bestAxis, _obb.origin) - minPenetration * 0.5f;
    float planeMax = glm::dot(bestAxis, _obb.origin) + minPenetration * 0.5f;

    for (int i = 0; i < 3; ++i)
    {
      glm::vec3 p(_triangle[i][0], _triangle[i][1], _triangle[i][2]);
      float d = glm::dot(bestAxis, p);
      if (d >= planeMin && d <= planeMax)
        result.contacts.push_back(p);
    }

    // If no vertices inside overlap region, fall back to center of triangle
    if (result.contacts.empty())
      result.contacts.push_back((triangle_p1 + triangle_p2 + triangle_p3) / 3.0f);
  ...
  //-------------------------------------------------------------
  CollisionManifold TriangleEllipse(const dbb::triangle& t, const dbb::ellipse& e)
  {
    CollisionManifold result;
    result.colliding = false;

    // Step 1: Construct transform matrix from ellipsoid local to world
    glm::mat3 scaleMatrix = glm::mat3(1.0f);
    scaleMatrix[0][0] = e.radii.x;
    scaleMatrix[1][1] = e.radii.y;
    scaleMatrix[2][2] = e.radii.z;

    glm::mat3 ellipsoidTransform = e.orientation * scaleMatrix;

    // Step 2: Inverse transform (world to unit sphere space)
    glm::mat3 invTransform = glm::inverse(ellipsoidTransform);

    auto ToUnitSphereSpace = [&](const glm::vec3& p) -> glm::vec3 {
      return invTransform * (p - e.center);
    };

    auto ToWorldSpace = [&](const glm::vec3& p) -> glm::vec3 {
      return ellipsoidTransform * p + e.center;
    };

    // Step 3: Transform triangle vertices and ellipsoid center into unit sphere space
    dbb::triangle t_unit = {
        ToUnitSphereSpace(t[0]),
        ToUnitSphereSpace(t[1]),
        ToUnitSphereSpace(t[2])
    };

    glm::vec3 sphereCenter_unit = glm::vec3(0.0f); // Ellipsoid center becomes origin
    float radius = 1.0f;

    // Step 4: Run triangle-sphere intersection in unit space
    dbb::sphere sphere(sphereCenter_unit, radius);
    if (CollisionManifold sp_collision = TriangleSphere(t_unit, sphere); !sp_collision.colliding)
      return result;
    else
    {
      // Step 5: Transform contact info back to world space
      glm::vec3 contactWorld = ToWorldSpace(sp_collision.contacts.back());
      glm::vec3 normalWorld = glm::normalize(glm::transpose(invTransform) * sp_collision.normal);

      result.colliding = true;
      result.contacts.push_back(contactWorld);
      result.normal = normalWorld;
      result.depth = sp_collision.depth * glm::length(ellipsoidTransform * sp_collision.normal); // Scale penetration
      return result;
    }
  }
  ...
  {
    size_t points_count = _mesh.GetIndices().size();
    const std::vector<Vertex>& vertices = _mesh.GetVertexs();
    const std::vector<unsigned int>& indices = _mesh.GetIndices();
    for (int i = 0; i < points_count; i += 3)
    {
      dbb::triangle tr = { {vertices[indices[i]].Position}, {vertices[indices[i + 1]].Position}, {vertices[indices[i + 2]].Position} };
      if (auto collision = TriangleOBB(tr, _obb); collision.colliding)
        return true;
    }
    return false;
  }
  ...
  bool MeshSphere(const I3DMesh& _mesh, const dbb::sphere& _s)
  {
    ...
    {
      dbb::triangle tr = { {vertices[indices[i]].Position}, {vertices[indices[i + 1]].Position}, {vertices[indices[i + 2]].Position} };
      if (auto collision = TriangleSphere(tr, _s); collision.colliding)
        return true;
    }
    return false;
  }
  ...
  //---------------------------------------------------------------------------------
  CollisionManifold TriangleSphere(const dbb::triangle& _triangle, const dbb::sphere& _sphere)
  {
    CollisionManifold collision;
    // Step 1: Find closest point on triangle to the sphere center
    dbb::point closest = GetClosestPointOnTriangle(_triangle, _sphere.position);

    // Step 2: Vector from triangle to sphere
    glm::vec3 v = _sphere.position - closest;
    float distSquared = glm::length2(v);
    ...
    // Step 4: Compute contact normal
    if (dist > 1e-6f) {
      collision.normal = glm::normalize(v); // from triangle to sphere
    }
    else {
      // Sphere center is exactly on the triangle or very close; use triangle normal
      dbb::plane plane(_triangle);
      collision.normal = plane.Normal();
    }

    // Step 5: Set outputs
    collision.contacts.push_back(closest);
    collision.depth = radius - dist;
  ...
  CollisionManifold FindCollision(const dbb::OBB& A, const dbb::sphere& B)
  {
    ...
    glm::vec3 delta = B.position - closestPoint;
    ...
    {
      // Sphere center is inside the box or exactly on the surface
      // Try to generate a reliable normal
      glm::vec3 fallback = glm::normalize(B.position - A.origin);
      if (glm::length2(fallback) < 0.0001f)
        fallback = glm::vec3(1, 0, 0); // Arbitrary fallback if fallback fails
    ...
    float penetrationDepth = glm::length(closestPoint - outsidePoint);

    result.colliding = true;
    result.normal = normal;
    result.depth = penetrationDepth * 0.5f;

    // Contact at midpoint between outside point and closest point
  ...
  CollisionManifold FindCollision(const OBB& A, const OBB& B)
  {
    ...
    // ============================================
    // FILTER: Reduce contact count for edge-face
    // ============================================
    //if (result.contacts.size() > 2)
    //{
    //  // If bestAxis is in range [0, 2] or [3, 5], it's a face normal -> likely face-vs-edge
    //  if (bestAxis >= 0 && bestAxis <= 5)
    //  {
    //    // Find the two contacts with the greatest distance apart
    //    float maxDistSq = 0.0f;
    //    int idxA = 0, idxB = 0;

    //    for (int i = 0; i < result.contacts.size(); ++i)
    //    {
    //      for (int j = i + 1; j < result.contacts.size(); ++j)
    //      {
    //        float distSq = glm::length2(result.contacts[i] - result.contacts[j]);
    //        if (distSq > maxDistSq)
    //        {
    //          maxDistSq = distSq;
    //          idxA = i;
    ...
    //    if (maxDistSq > 0.00001f)
    //    {
    //      glm::vec3 a = result.contacts[idxA];
    //      glm::vec3 b = result.contacts[idxB];
    //      result.contacts.clear();
    //      result.contacts.push_back(a);
    //      result.contacts.push_back(b);
    //    }
    //    else
    //    {
    //      // Keep only one if all are too close
  ...
  //--------------------------------------------------------------------------------
  CollisionManifold FindCollision(const I3DMesh& _mesh, const ITransform& _mesh_transform, const OBB& _obb)
  {
    CollisionManifold result;
    CollisionManifold::ResetCollisionManifold(result);
    ...
    const std::vector<Vertex>& vertices = _mesh.GetVertexs();
    std::vector<unsigned int> indices = _mesh.GetIndices();
    glm::mat4 world_matrix = _mesh_transform.getModelMatrix();
    for (int i = 0; i < points_count; i += 3)
    {
      glm::vec4 dot1 = world_matrix* glm::vec4(vertices[indices[i]].Position, 1.);
      glm::vec4 dot2 = world_matrix * glm::vec4(vertices[indices[i + 1]].Position, 1.);
      glm::vec4 dot3 = world_matrix * glm::vec4(vertices[indices[i + 2]].Position, 1.);
      dbb::triangle tr = { dot1, dot2, dot3 };
      if (CollisionManifold res = TriangleOBB(tr, _obb); res.colliding)
      {
        return res;
      }
    }
  ...
  //------------------------------------------------------------------------------------
  CollisionManifold FindCollision(const I3DMesh& _mesh, const dbb::sphere& _sphere)
  {
    CollisionManifold result;
    CollisionManifold::ResetCollisionManifold(result);
    ...
    {
      dbb::triangle tr = { {vertices[indices[i]].Position}, {vertices[indices[i + 1]].Position}, {vertices[indices[i + 2]].Position} };
      if (CollisionManifold res = TriangleSphere(tr, _sphere); res.colliding)
      {
        return res;
      }
    }
  ...
  //-----------------------------------------------------------------------
  CollisionManifold FindCollision(const I3DMesh& _mesh, const dbb::ellipse& _ellipse)
  {
    CollisionManifold result;
    CollisionManifold::ResetCollisionManifold(result);
    ...
    {
      dbb::triangle tr = { {vertices[indices[i]].Position}, {vertices[indices[i + 1]].Position}, {vertices[indices[i + 2]].Position} };
      if (CollisionManifold res = TriangleEllipse(tr, _ellipse); res.colliding)
      {
        return res;
      }
    }
