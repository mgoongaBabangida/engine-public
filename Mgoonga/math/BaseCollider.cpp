...
//-------------------------------------------------------------------------------
void BaseCollider::CalculateExtremDots(IModel* _model)
{
	...
	m_center = BaseCollider::GetCenterLocalSpace();
	m_radius = BaseCollider::GetRadius();
	m_model_name = _model->GetName();
}

//-------------------------------------------------------------------------------
void BaseCollider::CalculateExtremDots(const eObject* _object)
{
...
//-------------------------------------------------------------------------------
std::vector<glm::mat3> BaseCollider::GetBoundingTriangles(const ITransform& trans) const
{
...
//-------------------------------------------------------------------------------
std::vector<glm::vec3> BaseCollider::GetExtrems(const ITransform& trans) const
{
	glm::mat4 transform = trans.getModelMatrix();
...
//-------------------------------------------------------------------------------
std::vector<glm::mat3> BaseCollider::GetBoundingTrianglesLocalSpace() const
{
	//extrem dots should exist
	std::vector<glm::mat3> ret; // Getting 12 triangles of the bouning cube
...
//-------------------------------------------------------------------------------
std::vector<glm::vec3> BaseCollider::GetExtremsLocalSpace() const
{
	std::vector<glm::vec3> ret;
...
//-------------------------------------------------------------------------------
glm::vec3 BaseCollider::GetCenterLocalSpace() const
{
...
//-------------------------------------------------------------------------------
float BaseCollider::GetRadius() const
{
	if (m_radius == 0.0f)
	{
...
//---------------------------------------------------------------------------
float BaseCollider::GetScaledRadius() const
{
	return GetRadius() * m_scale;
}

//-------------------------------------------------------------------------------
void BaseCollider::SetScale(float _scale)
{
	m_scale = _scale;
}

//-------------------------------------------------------------------------------
std::vector<dbb::line> BaseCollider::_GetRays(const ITransform & trans, Side moveDirection, std::vector<float>& lengths)
{
...
//-------------------------------------------------------------------------------
void BaseCollider::_GetForwardRayLengths(const ITransform & trans, Side moveDirection, std::vector<float>& lengths) const
{
		if (lengths.empty())
...
//--------------------------------------------------------------------------------------------
bool BaseCollider::CollidesWith(const ITransform& _trans,
															 ...
															 eCollision& _collision)
{
	...
	glm::vec3 center = _trans.getModelMatrix() * glm::vec4(BaseCollider::GetCenterLocalSpace(), 1.0f);
...
//-------------------------------------------------------------------
bool BaseCollider::CollidesWith(const ITransform& _trans, std::vector<shObject> _objects, Side _moveDirection, eCollision& _collision)
{
...
//----------------------------------------------------------------------------------------
bool BaseCollider::IsInsideOfAABB(const ITransform& _trans, const ITransform& _trans2, const ICollider& _other)
{
...
//----------------------------------------------------------------------------------------
dbb::OBB BaseCollider::_GetOBB(const ITransform& _trans) const
{
	dbb::OBB obb;
	obb.origin = _trans.getModelMatrix() * glm::vec4(BaseCollider::GetCenterLocalSpace(), 1.0f);
...
//----------------------------------------------------------------------------------------
dbb::sphere BaseCollider::_GetSphere(const ITransform& _trans) const
{
	dbb::sphere sphere;
...
//----------------------------------------------------------------------------------------
bool BaseCollider::_CheckByRadius(const ITransform& _trans, const ITransform& _trans_other, ICollider* _other)
{
	glm::vec4 center				= _trans.getModelMatrix()				* glm::vec4(BaseCollider::GetCenterLocalSpace(), 1.0f);
	glm::vec4 other_center	= _trans_other.getModelMatrix() * glm::vec4(_other->GetCenterLocalSpace(), 1.0f);
