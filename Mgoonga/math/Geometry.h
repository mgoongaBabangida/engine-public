...
namespace dbb
{
  ...
  struct DLL_MATH OBB
  {
    ...
    inline OBB() : origin(0), size(0, 0, 0), orientation(1.0f) { } //!isValid()
    inline OBB(const dbb::point& p, const glm::vec3& s) : origin(p), size(s), orientation(1.0f) { }
    ...
    std::array<unsigned int, 24>  GetIndices() const;
