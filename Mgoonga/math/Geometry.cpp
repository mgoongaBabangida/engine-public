...
#include <glm\glm/glm.hpp>            // Core types like vec3, mat3
#include <glm\glm/gtx/component_wise.hpp> // For glm::isfinite
#include <glm\glm/gtc/epsilon.hpp>    // For epsilon-related functions (optional)

namespace dbb
{
  //----------------------------------------------------
  inline bool isVec3Finite(const glm::vec3& v)
  {
  ...
  float ray::Raycast(const OBB& obb, RaycastResult& outResult) const
  {
    ...
    if (tmax < 0)
      return -1.0f;  // OBB is behind the ray
    if (tmin < 0.0f)
      return tmax;   // Ray origin is inside OBB
  ...
  std::array<unsigned int, 24> OBB::GetIndices() const
  {
      return { 0, 1, 1, 7, 7, 2, 2, 0,
               6, 4, 4, 5, 5, 3, 3, 6,
               3, 0, 5, 2, 6, 1, 4, 7 };
  }
  ...
  {
    // Size must be all positive and finite
    ...
    if (!isVec3Finite(origin)) {
      return false;
    }

    // Orientation matrix must be orthonormal (basis vectors of unit length and orthogonal)
    const float epsilon = 1e-4f;
    glm::vec3 x = orientation[0];
    glm::vec3 y = orientation[1];
    glm::vec3 z = orientation[2];
    ...
    if (std::abs(glm::length(x) - 1.0f) > epsilon ||
    ...
    if (std::abs(glm::dot(x, y)) > epsilon ||
