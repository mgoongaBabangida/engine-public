...
namespace dbb
{
  ...
  {
  public:
    base::Event<std::function<void(const CollisionPair&)>>  CollisionOccured;

    void Update(float _deltaTime); // called in main thread aligned with 
  ...
  protected:
    ...
    float                                          m_linearProjectionPercent = 0.15f;
