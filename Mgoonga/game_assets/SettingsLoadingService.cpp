...
//-------------------------------------------------------------------------------------------------
void SettingsLoadingService::LoadModelTextures(const std::string& _path, ModelManagerYAML* _modelManager)
{
	std::ifstream infile(_path);
	if (infile.is_open())
	{
		...
		std::string name, index, diffuse_path, specular_path, normal_path, end;
		while (!sstream.eof())
		{
			sstream >> name;
			sstream >> index;
			sstream >> diffuse_path;
			sstream >> specular_path;
			sstream >> normal_path;
			sstream >> end;
			if(name.empty())
				continue;
			auto model = _modelManager->Find(name);
			if (model.get() == nullptr)
				continue;
			int inx = std::atoi(index.c_str());
			...
			if (diffuse_path != "-")
			{
				...
				t.loadTextureFromFile(model->GetPath() + "/" + diffuse_path);
				const_cast<I3DMesh*>(model->Get3DMeshes()[inx])->AddTexture(&t);
			}

			if (specular_path != "-")
			{
				t.m_type = "texture_specular";
				t.loadTextureFromFile(model->GetPath() + "/" + specular_path);
				const_cast<I3DMesh*>(model->Get3DMeshes()[inx])->AddTexture(&t);
			}
			...
			if (normal_path != "-")
			{
				...
				t.loadTextureFromFile(model->GetPath() + "/" + normal_path);
				const_cast<I3DMesh*>(model->Get3DMeshes()[inx])->AddTexture(&t);
			}
		}
	}
