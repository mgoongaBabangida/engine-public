																													...
																													eOpenGlRenderPipeline& _pipeline)
{
	...
	static size_t g_texture_index = 0;
	std::vector<std::string> atlas_names = _texManager->GetTextureAtlasesNames();
	static Texture g_current_texture = *_texManager->Find(atlas_names[g_texture_index]);

	m_psystem = std::make_shared<ParticleSystem>(50, 1.0f/50.0f, 0, 10'000, glm::vec3(0.5f, 3.0f, -2.5f),
																							&g_current_texture,
																							_soundManager->GetSound("shot_sound"),
																							g_current_texture.m_num_rows);
	...
	static std::function<size_t(size_t)> objects_callback = [this, _texManager, atlas_names](size_t _index)
	{
		size_t prev = g_texture_index;
		if (_index != MAXSIZE_T)
		{
			g_texture_index = _index;
			if (const Texture* p_atlas = _texManager->Find(atlas_names[g_texture_index]); p_atlas)
			{
				g_current_texture = *p_atlas;
			}
		}
		return prev;
	};
	static eVectorStringsCallback watcher{ atlas_names, objects_callback };
	_imgui->Add(COMBO_BOX, "Atlas", &watcher);

	_imgui->Add(TEXTURE_POINTER, "Texture", (void*)&g_current_texture.m_id);
