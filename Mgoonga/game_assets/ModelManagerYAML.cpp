...
//------------------------------------------------------------------------
void  ModelManagerYAML::Add(const std::string& _name, std::shared_ptr<MyModel> _model)
{
  eModelManager::Add(_name, _model);
}
...
IModel* ModelManagerYAML::_LoadModel(const std::string& _name, const std::string& _filepath)
{
  ...
  m_models.insert(std::pair<std::string, std::shared_ptr<IModel> >(_name, model));
  return model;
}
