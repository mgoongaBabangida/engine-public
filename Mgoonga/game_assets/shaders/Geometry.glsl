...
void build_hex_flat(vec4 position)
{    
    gl_Position = position +  MVP * vec4(0.5, 0.0, 0.866, 0.0) * radius ;    // 1:bottom-left
    EmitVertex();   
    gl_Position = position +  MVP * vec4( 1.0, 0.0, 0.0,  0.0) * radius;    // 2:bottom-right
    EmitVertex();
    gl_Position = position +  MVP * vec4(0.5, 0.0,  -0.866,  0.0) * radius;    // 3:top-left
    EmitVertex();
    gl_Position = position +  MVP * vec4( -0.5, 0.0, -0.866,  0.0) * radius;    // 4:top-right
    EmitVertex();
    gl_Position = position +  MVP * vec4( -1.0, 0.0, 0.0,  0.0) * radius;    // 5:top
    EmitVertex();
	gl_Position = position +  MVP * vec4( -0.5, 0.0, 0.866,  0.0) * radius;    // 6:top
    EmitVertex();
	gl_Position = position +  MVP * vec4(0.5, 0.0, 0.866, 0.0) * radius;     // 7:top
    EmitVertex();
    EndPrimitive();
}
...
{    
	...
	if(type == 0)
		...
		build_hex_flat(gl_in[0].gl_Position);
} 
