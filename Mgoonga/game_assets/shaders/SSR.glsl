...
layout(location = 1) out float SSRMask;
...
{
   vec3 originalDir = dir;           // preserve full original step direction
    float searchStep = 1.0;
    float depth;
    vec4 projectedCoord;

    for (int i = 0; i < numBinarySearchSteps; ++i)
    {
        searchStep *= 0.5;            // halve step size each iteration
        vec3 offset = originalDir * searchStep;

        vec3 forward = hitCoord + offset;

        projectedCoord = proj * vec4(forward, 1.0);
        ...
        depth = texture(tPos, projectedCoord.xy).z;
        dDepth = forward.z - depth;

        if (dDepth > 0.0)
            hitCoord = forward; // still in front of surface
        // else: keep previous hitCoord
    }
    ...
    dir = originalDir * searchStep; // preserve contract of 'dir' being halved overall
...
vec4 RayMarch(vec3 dir, inout vec3 hitCoord, out float dDepth)
{
    float depth;
    vec4 projectedCoord;
    float stepLength = step * clamp(-hitCoord.z * 0.05, 0.1, 5.0);
...
void main()
{
	...
	float materialMask = texture(tMask, TexCoords).r;
    ...
    float ReflectionMultiplier = pow(Metallic, reflectionSpecularFalloffExponent) * 
 	...
 	//SSRMask = coords.w > 0.0 ? pow(ReflectionMultiplier, 0.6) : 0.0;
	SSRMask = coords.w > 0.0 ? 1.0 : 0.0;
	if (materialMask < 0.01)
		SSRMask = 0.0;
