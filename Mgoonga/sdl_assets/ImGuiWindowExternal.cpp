...
void eWindowImGuiExternal::Render()
{
  ...
  for (auto& item : lines)
  {
    ...
    case SLIDER_FLOAT_SPIN:
    {
      ImGui::Text(std::get<0>(item).c_str());
      float* f = static_cast<float*>(std::get<2>(item));
      ImGui::SliderFloat(("##"+ std::get<0>(item)).c_str(), f, -10.0f, 10.0f); // -10 10 @todo
      ImGui::SameLine();
      if (ImGui::Button(("+##" + std::get<0>(item)).c_str())) { *f += 0.1f; }
      ImGui::SameLine();
      if (ImGui::Button(("-##" + std::get<0>(item)).c_str())) { *f -= 0.1f; }
    } break;
    case SLIDER_FLOAT_P:        ImGui::SliderFloat(std::get<0>(item).c_str(), *(static_cast<float**>(std::get<2>(item))), -10.0f, 10.0f); break;
    case SLIDER_FLOAT_NERROW:   ImGui::SliderFloat(std::get<0>(item).c_str(), static_cast<float*>(std::get<2>(item)), -1.0f, 1.0f); break;
    case SLIDER_FLOAT_NERROW_P: ImGui::SliderFloat(std::get<0>(item).c_str(), *(static_cast<float**>(std::get<2>(item))), -1.0f, 1.0f); break;
    ...
    case TEXT_VEC:
    {
      ImGui::Text(std::get<0>(item).c_str());  ImGui::SameLine();
      glm::vec3* transfer_float = static_cast<glm::vec3*>(std::get<2>(item));
      std::string value_x = std::to_string(transfer_float->x);
      ImGui::Text(value_x.c_str()); ImGui::SameLine();
      std::string value_y = std::to_string(transfer_float->y);
      ImGui::Text(value_y.c_str()); ImGui::SameLine();
      std::string value_z = std::to_string(transfer_float->z);
      ImGui::Text(value_z.c_str()); 
    }
    break;
    case TEXT_FLOAT:
    {
      ImGui::Text(std::get<0>(item).c_str());  ImGui::SameLine();
      float* transfer_float = static_cast<float*>(std::get<2>(item));
      ImGui::Text(std::to_string(*transfer_float).c_str());
    }
    break;
    case TEXT_FLOAT_P:
    {
    ...
    {
      ImGui::Text(std::get<0>(item).c_str());
      GLuint* id_ptr = static_cast<GLuint*>(std::get<2>(item));
      ImGui::Image((void*)(*id_ptr), ImVec2(240, 160), ImVec2(0, 1), ImVec2(1, 0));
    }
    break;
    case SLIDER_FLOAT_3:
    {
    ...
    case COLOR_PICKER:
    {
      glm::vec3* p_color = static_cast<glm::vec3*>(std::get<2>(item));
      ImGui::ColorEdit3(std::get<0>(item).c_str(), &p_color->x);
    }
    break;
    case COMBO_BOX:
    {
      ...
      if (!cstrings.empty())
      {
        if (ImGui::Combo(std::get<0>(item).c_str(), &transfer_data->current_item, &cstrings[0], transfer_data->data.size()))
          transfer_data->callback(transfer_data->current_item);
      }
      transfer_data->current_item = transfer_data->callback(MAXSIZE_T);
    }
    break;
    case TEXTURE_ARRAY:
    {
    ...
    case LIGHT_TYPE_VISUAL:
    {
      ...
      {
        if (obj.get() && obj->Name() == "LightObject")
          lightObj = obj;
      }
      ...
      if (current_type_item == NULL && lightObj.get())
      {
        ...
        else if (lightObj->GetModel()->GetName() == "white_quad" && !lightObj->GetModel()->GetMaterial()->used_textures.contains(Material::TextureType::ALBEDO))
      ...
      else if (current_type_item == types[1].c_str()) // logic if choice is quad (area light etc.)
      {
        if (std::shared_ptr<IModel> model = modelManager->Find("white_quad"); model.get() != nullptr)
        {
          ...
          m.used_textures.erase(Material::TextureType::ALBEDO);
          m.textures[Material::TextureType::EMISSIVE] = Texture::GetTexture1x1(WHITE).m_id;
          lightObj->GetModel()->SetMaterial(m);
        }
      }
      else if (current_type_item == types[2].c_str())
      {
        lightObj->SetTransparent(false);
        if (std::shared_ptr<IModel> model = modelManager->Find("white_quad"); model.get() != nullptr)
        {
          if (lightObj->GetModel() != model.get()) // if the model is not yet correct
          {
          ...
          lightObj->GetTransform()->setScale(glm::vec3(0.3f, 0.3f, 0.3f));
          Material m = lightObj->GetModel()->GetMaterial().value();
          m.textures[Material::TextureType::ALBEDO] = Texture::GetLightIconTextureId();
          m.used_textures.insert(Material::TextureType::ALBEDO);
          m.textures[Material::TextureType::EMISSIVE] = Texture::GetTexture1x1(BLACK).m_id;
          lightObj->GetModel()->SetMaterial(m);
        }
      }
    ...
    case OBJECT_REF_MATERIAL:
    {
      ...
      if (p_object && *p_object)
      {
        ...
        // Material
        Material material;
        static bool use_albedo = false;
        static bool use_metalic = false;
        static bool use_normal = false;
        static bool use_roughness = false;
        static bool use_ao = false;
        ...
        use_albedo = material.used_textures.contains(Material::TextureType::ALBEDO);
        use_metalic = material.used_textures.contains(Material::TextureType::METALLIC);
        use_normal = material.used_textures.contains(Material::TextureType::NORMAL);
        use_roughness = material.used_textures.contains(Material::TextureType::ROUGHNESS);
        use_ao = material.used_textures.contains(Material::TextureType::AO);
        ...
        texture_id = material.textures[Material::TextureType::ALBEDO] != Texture::GetDefaultTextureId()
                   ? material.textures[Material::TextureType::ALBEDO] : Texture::GetEmptyTextureId();
        ...
        ImGui::Checkbox("Use albedo texture", &use_albedo);

        ImGui::Text("Metalic texture");
        texture_id = material.textures[Material::TextureType::METALLIC] != Texture::GetDefaultTextureId()
          ? material.textures[Material::TextureType::METALLIC] : Texture::GetEmptyTextureId();
        
        ImGui::Image((void*)(intptr_t)(texture_id), ImVec2(240, 160), ImVec2(0, 1), ImVec2(1, 0));
        ImGui::SliderFloat("Metalic", &material.metallic, 0.0f, 1.0f);
        ImGui::Checkbox("Use metalic texture", &use_metalic);

        ImGui::Text("Normal texture");
        texture_id = material.textures[Material::TextureType::NORMAL] != Texture::GetDefaultTextureId()
          ? material.textures[Material::TextureType::NORMAL] : Texture::GetEmptyTextureId();

        ImGui::Image((void*)(intptr_t)(texture_id), ImVec2(240, 160), ImVec2(0, 1), ImVec2(1, 0));
        ImGui::Checkbox("Use normal map texture", &use_normal);

        ImGui::Text("Roughness texture");//contra glossiness
        texture_id = material.textures[Material::TextureType::ROUGHNESS] != Texture::GetDefaultTextureId()
          ? material.textures[Material::TextureType::ROUGHNESS] : Texture::GetEmptyTextureId();

        ImGui::Image((void*)(intptr_t)(texture_id), ImVec2(240, 160), ImVec2(0, 1), ImVec2(1, 0));
        ImGui::SliderFloat("Roughness", &material.roughness, 0.0f, 1.0f);
        ImGui::Checkbox("Use roughness texture", &use_roughness);

        ImGui::Text("AO texture");
        texture_id = material.textures[Material::TextureType::AO] != Texture::GetDefaultTextureId()
          ? material.textures[Material::TextureType::AO] : Texture::GetEmptyTextureId();

        ImGui::Image((void*)(intptr_t)(texture_id), ImVec2(240, 160), ImVec2(0, 1), ImVec2(1, 0));
        ImGui::Checkbox("Use ao map texture", &use_ao);
        ImGui::SliderFloat("AO", &material.ao, 0.0f, 1.0f);

        ImGui::Checkbox("Set Transparent", &obj->GetTransparent());
        ImGui::SliderFloat("Opacity", &material.opacity, 0.0f, 1.0f);

        ImGui::Text("Emissive texture mask");
        texture_id = material.textures[Material::TextureType::EMISSIVE] != Texture::GetDefaultTextureId()
          ? material.textures[Material::TextureType::EMISSIVE] : Texture::GetEmptyTextureId();

        ImGui::Image((void*)(intptr_t)(texture_id), ImVec2(240, 160), ImVec2(0, 1), ImVec2(1, 0));
        
        ImGui::ColorPicker3("Emission color", &material.emission_color.x);

        ImGui::Text("Emission strength");
        ImGui::SliderFloat("Emissive", &material.emission_strength, 0.0f, 20.0f);

        //@todo add displacement/ bump as separate slot

        if (use_albedo) material.used_textures.insert(Material::TextureType::ALBEDO); else material.used_textures.erase(Material::TextureType::ALBEDO);
        if (use_metalic) material.used_textures.insert(Material::TextureType::METALLIC); else material.used_textures.erase(Material::TextureType::METALLIC);
        if (use_normal) material.used_textures.insert(Material::TextureType::NORMAL); else material.used_textures.erase(Material::TextureType::NORMAL);
        if (use_roughness) material.used_textures.insert(Material::TextureType::ROUGHNESS); else material.used_textures.erase(Material::TextureType::ROUGHNESS);
        if (use_ao) material.used_textures.insert(Material::TextureType::AO); else material.used_textures.erase(Material::TextureType::AO);
    ...
    case OBJECT_REF_RIGGER:
    {
      ...
      static const char* current_animation_blend_from_item = NULL;
      static const char* current_animation_blend_to_item = NULL;
      static RigAnimator* g_rigger = nullptr;
      static int cur_frame = 0;
      static int cur_frame_blend_from_item = 0;
      static int cur_frame_blend_to_item = 0;
      static bool g_fix_hips_z = false;
      static bool g_fix_hips_y = false;
      ...
      if (p_object && *p_object)
      {
        if (*p_object != m_current_object || (*p_object)->GetRigger() != g_rigger)
        {
          ...
          {
            RigAnimator* rigger = dynamic_cast<RigAnimator*>(m_current_object->GetRigger());
            animation_names = rigger->GetAnimationNames();
            if (!animation_names.empty())
            {
              current_animation_item = animation_names[0].c_str();
              current_animation_blend_from_item = animation_names[0].c_str();
              current_animation_blend_to_item = animation_names[0].c_str();
        ...
        if (obj->GetRigger())
        {
          ...
          ImGui::Text(text.c_str());
          text = std::string("Hips Bone: ") + rigger->GetNameHipsBone();
          ImGui::Text(text.c_str());
          ...
          if (rigger && rigger->GetCurrentAnimation())
          {
            g_fix_hips_z = rigger->GetCurrentAnimation()->GetFixHipsZ();
            ImGui::Checkbox("Fix hips Z", &g_fix_hips_z);
            rigger->GetCurrentAnimation()->GetFixHipsZ() = g_fix_hips_z;

            g_fix_hips_y = rigger->GetCurrentAnimation()->GetFixHipsY();
            ImGui::Checkbox("Fix hips Y", &g_fix_hips_y);
            rigger->GetCurrentAnimation()->GetFixHipsY() = g_fix_hips_y;

            //arm offset
            ImGui::SliderFloat("Arm Offset", &rigger->GetArmOffset(), 0.0f, 45.0f);

            //speed
            ImGui::SliderFloat("Speed", &rigger->GetCurrentAnimation()->GetSpeed(), 0.0f, 5.0f);

            //duration combo
            int* duration_combo_value = nullptr;
            duration_combo_value = &rigger->GetCurrentAnimation()->GetDuration();
            if (ImGui::InputInt("Duration msc", duration_combo_value)) // The second parameter is the label previewed before opening the combo.
            {
            }
          }
        ...
        if (ImGui::InputText("Load Rigger from", &load_path[0], 100, ImGuiInputTextFlags_EnterReturnsTrue))
        {

        }
        if (ImGui::Button("Load Rigger"))
        {
        ...
        if (ImGui::InputText("Save Rigger to", &save_path[0], 100, ImGuiInputTextFlags_EnterReturnsTrue))
        {

        }
        if (ImGui::Button("Save Rigger"))
        {
          if (auto it = callbacks_string.find("Save Rigger"); it != callbacks_string.end())
            it->second(m_current_object, save_path);
        }

        static std::string save_animations_path(150, '\0');
        if (ImGui::InputText("Save Animations to", &save_animations_path[0], 100, ImGuiInputTextFlags_EnterReturnsTrue))
        {

        }
        if (ImGui::Button("Save Animations"))
        {
          if (auto it = callbacks_string.find("Save Animations"); it != callbacks_string.end())
            it->second(m_current_object, save_animations_path);
        }
        ...
        static std::string load_collider_path(150, '\0');
        if (ImGui::InputText("Load Dynamic Collider from", &load_collider_path[0], 100, ImGuiInputTextFlags_EnterReturnsTrue))
        {

        }
        if (ImGui::Button("Load Dynamic Collider"))
        {
          if (auto it = callbacks_string.find("Load Dynamic Collider"); it != callbacks_string.end())
            it->second(m_current_object, load_collider_path);
        }

        static std::string save_collider_path(150, '\0');
        if (ImGui::InputText("Save Dynamic Collider to", &save_collider_path[0], 100, ImGuiInputTextFlags_EnterReturnsTrue))
        {

        }
        if (ImGui::Button("Save Dynamic Collider"))
        {
          if (auto it = callbacks_string.find("Save Dynamic Collider"); it != callbacks_string.end())
            it->second(m_current_object, save_collider_path);
        }
        ...
        if (ImGui::Button("Create Socket"))
        {
          if (current_game_object_item)
          {
            ...
            glm::mat4 pretransform = glm::inverse(m_current_object->GetTransform()->getModelMatrix()) * child->GetTransform()->getModelMatrix();
            m_current_object->GetRigger()->CreateSocket(child, current_bone_item, pretransform, false);
            // to test correct global mapping @todo
            //glm::mat4 parentM = m_current_object->GetTransform()->getModelMatrix();
            //glm::mat4 boneBindG = m_current_object->GetRigger()->GetGlobalBindMatrixForBone(current_bone_item);
            //glm::mat4 childW = child->GetTransform()->getModelMatrix();

            //glm::mat4 pre = glm::inverse(parentM * boneBindG) * childW;
            //m_current_object->GetRigger()->CreateSocket(child, current_bone_item, pre, /*map_global=*/true);
          }
        }
        if (ImGui::Button("Delete Sockets"))
        {
          m_current_object->GetRigger()->ClearSockets();
        }

        // Animation Blending
        if (m_current_object && m_current_object->GetRigger())
        {
          RigAnimator* rigger = dynamic_cast<RigAnimator*>(m_current_object->GetRigger());
          if (current_animation_blend_from_item != NULL && animation_names.size() > 1 && !animation_names[0].empty())
          {
            if (ImGui::BeginCombo("Animation to bled from", current_animation_blend_from_item)) // The second parameter is the label previewed before opening the combo.
            {
              for (int n = 0; n < animation_names.size(); ++n)
              {
                bool is_selected = (current_animation_blend_from_item == animation_names[n].c_str()); // You can store your selection however you want, outside or inside your objects
                if (ImGui::Selectable(animation_names[n].c_str(), is_selected))
                {
                  current_animation_blend_from_item = animation_names[n].c_str();
                }
                if (is_selected)
                {
                  ImGui::SetItemDefaultFocus();   // You may set the initial focus when opening the combo (scrolling + for keyboard navigation support)
          ...
          int* value_blend_from = nullptr;
          value_blend_from = &cur_frame_blend_from_item;
          if (ImGui::InputInt("Frame to blend from", value_blend_from)) // The second parameter is the label previewed before opening the combo.
          {}

          if (current_animation_blend_to_item != NULL && animation_names.size() > 1 && !animation_names[0].empty())
          {
            if (ImGui::BeginCombo("Animation to bled to", current_animation_blend_to_item)) // The second parameter is the label previewed before opening the combo.
            {
              for (int n = 0; n < animation_names.size(); ++n)
              {
                bool is_selected = (current_animation_blend_to_item == animation_names[n].c_str()); // You can store your selection however you want, outside or inside your objects
                if (ImGui::Selectable(animation_names[n].c_str(), is_selected))
                {
                  current_animation_blend_to_item = animation_names[n].c_str();
                }
                if (is_selected)
                {
                  ImGui::SetItemDefaultFocus();   // You may set the initial focus when opening the combo (scrolling + for keyboard navigation support)
          ...
          //frame to interpolate
          int* value_blend_to = nullptr;
          value_blend_to = &cur_frame_blend_to_item;
          if (ImGui::InputInt("Frame to blend to", value_blend_to)) // The second parameter is the label previewed before opening the combo.
          {
          }

          if (ImGui::Button("Create Transit Animation"))
          {
            std::shared_ptr<SkeletalAnimation> transit = std::make_shared<SkeletalAnimation>(SkeletalAnimation::CreateByInterpolation(
              rigger->GetAnimations()[std::string(current_animation_blend_from_item)]->GetFrameByNumber(cur_frame_blend_from_item),
              rigger->GetAnimations()[std::string(current_animation_blend_to_item)]->GetFrameByNumber(cur_frame_blend_to_item),
              500, // duration
              20, // num frames
              "Transit"));
    ...
    case OBJECT_LIST:
    {
      ...
      std::vector<shObject> objects = p_game->GetObjects();
      for (auto obj : objects)
      {
    ...
    case CAMERA:
    {
      ...
      // absolute "knob" values the user sees
      static float yawAbs = 0.0f;
      static float pitchAbs = 0.0f;
      static float rollAbs = 0.0f;

      // last-applied values, so we can compute deltas
      static float prevYawAbs = 0.0f;
      static float prevPitchAbs = 0.0f;
      static float prevRollAbs = 0.0f;
      ...
      ImGui::SliderFloat("Zoom", &p_camera->ZoomRef(), 30.f, 90.f);
      ImGui::SliderInt("Width", (int*)&p_camera->WidthRef(), 30, 960);
      ImGui::SliderInt("Height", (int*)&p_camera->HeightRef(), 30, 960);

      ImGui::PushItemWidth(ImGui::GetWindowWidth());
      ImGui::Text("Position");
      ImGui::SliderFloat3("Position Debug", &p_camera->PositionRef()[0], -10, 10);

      // --- Yaw
      {
        float old = yawAbs;
        if (ImGui::SliderFloat("Yaw", &yawAbs, -180.0f, 180.0f))
        {
          float delta = yawAbs - prevYawAbs;
          p_camera->rotateYaw(delta);
          prevYawAbs = yawAbs;
        }
        else
        {
          // if slider not active this frame, still sync prev to current if user dragged earlier
          // (optional safety if your UI code doesn't redraw every frame)
          prevYawAbs = yawAbs;
        }
      }
      ...
      {
        float old = pitchAbs;
        if (ImGui::SliderFloat("Pitch", &pitchAbs, -89.0f, 89.0f))
        {
          float delta = pitchAbs - prevPitchAbs;
          p_camera->rotatePitch(delta);
          prevPitchAbs = pitchAbs;
        }
        else
        {
          prevPitchAbs = pitchAbs;
      ...
      {
        float old = rollAbs;
        if (ImGui::SliderFloat("Roll", &rollAbs, -180.0f, 180.0f))
        {
          float delta = rollAbs - prevRollAbs;
          p_camera->rotateRoll(delta);
          prevRollAbs = rollAbs;
        }
        else
        {
      ...
      ImGui::Checkbox("Debug Frustum", &p_camera->VisualiseFrustum());
    ...
    case RADIO_BUTTON:
    {
      ...
      eVectorStringsCallback* transfer_data = static_cast<eVectorStringsCallback*>(std::get<2>(item));
      // Add radio buttons
      for (int i = 0; i < transfer_data->data.size(); ++i)
      {
        if (ImGui::RadioButton(transfer_data->data[i].c_str(), transfer_data->current_item == i)) {
          transfer_data->current_item = i;
          transfer_data->callback(transfer_data->current_item);
        }
        if(i != transfer_data->data.size()-1)
          ImGui::SameLine();
      }
    }
    break;
...
void eMainImGuiWindow::Render()
{
  ...
  */
  ...
  if (file_dialog.showFileDialog(open_file_menu_name.c_str(), imgui_addons::ImGuiFileBrowser::DialogMode::OPEN, ImVec2(700, 200), ".obj,.dae"))
  {
