...
//---------------------------------------------------------------
static uint32_t UiAsciiFromSDL(const SDL_Keysym& ks)
{
	// Keep common controls as ASCII
	switch (ks.sym)
	{
	case SDLK_TAB:			return ASCII_TAB;
	case SDLK_RETURN:		// main Enter
	case SDLK_KP_ENTER: return ASCII_ENTER;
	case SDLK_ESCAPE:		return ASCII_ESCAPE;
	case SDLK_SPACE:		return ASCII_SPACE;
	case SDLK_BACKSPACE:return ASCII_BACKSPACE;

		// Distinct navigation (>255)
	case SDLK_LEFT:   return ASCII_LEFT;
	case SDLK_RIGHT:  return ASCII_RIGHT;
	case SDLK_UP:     return ASCII_UP;
	case SDLK_DOWN:   return ASCII_DOWN;
	case SDLK_HOME:   return ASCII_HOME;
	case SDLK_END:    return ASCII_END;
	case SDLK_PAGEUP: return ASCII_PAGEUP;
	case SDLK_PAGEDOWN:return ASCII_PAGEDOWN;

	case	SDLK_0: return ASCII_0; // = '0',
	case	SDLK_1: return ASCII_1; // = '1',
	case	SDLK_2: return ASCII_2; // = '2',
	case	SDLK_3: return ASCII_3; // = '3',
	case	SDLK_4: return ASCII_4; // = '4',
	case	SDLK_5: return ASCII_5; // = '5',
	case	SDLK_6: return ASCII_6; // = '6',
	case	SDLK_7: return ASCII_7; // = '7',
	case	SDLK_8: return ASCII_8; // = '8',
	case	SDLK_9: return ASCII_9; // = '9',
	default: break;
	}

	// Printable ASCII fast path (keeps current behavior)
	// NOTE: This is layout-dependent (SDLK_* follows layout).
	if (ks.sym >= 32 && ks.sym < 127)
		return static_cast<uint32_t>(ks.sym);

	// Not mapped → 0 means "ignore" for now
	return 0;
}

//------------------------------------------------------------------------------
static KeyModifiers ModsFromSDL(SDL_Keymod s)
{
	KeyModifiers m = KeyModifiers::NONE;
	if (s & KMOD_CTRL)  m = (KeyModifiers)((int)m | (int)KeyModifiers::CTRL);
	if (s & KMOD_SHIFT) m = (KeyModifiers)((int)m | (int)KeyModifiers::SHIFT);
	if (s & KMOD_ALT)   m = (KeyModifiers)((int)m | (int)KeyModifiers::ALT);
	return m;
}
...
: inputController(), WIDTH(_width), HEIGHT(_height)
{
  ...
  guiWnd.push_back(new eWindowImGuiExternal("Object Transform"));//2
	...
	guiWnd.push_back(new eWindowImGuiExternal("Game Debug"));//10
	guiWnd.push_back(new eWindowImGuiExternal("Terrain Generation Tool"));//11
	guiWnd.push_back(new eWindowImGuiExternal("Particle System Tool"));//12
	...
	guiWnd.push_back(new eWindowImGuiExternal("Calculus Tool"));//16
	guiWnd.push_back(new eWindowImGuiExternal("Shadows"));//17
	guiWnd.push_back(new eWindowImGuiExternal("Deferred pipeline"));//18
  ...
  guiWnd[4]->Add(MENU, "Close", reinterpret_cast<void*>(&on_close));
	...
	static std::function<void(int, int*&)> min_frametime_callback = [this](int rate, int*& _val)
	{
		...
		dTimer->setPeriod(m_min_frametime);
	};
...
bool dbGLWindowSDL::InitializeGL()
{
	...
	SetOpenGLContextThreadId(std::this_thread::get_id());
...
void dbGLWindowSDL::Run()
{
	...
	while(running)
	{
		if(SDL_PollEvent(&windowEvent))
		{
			...
			const KeyModifiers mod = ModsFromSDL(sdlMods);

			if (windowEvent.type == SDL_KEYDOWN)
			{
				const SDL_Keysym& ks = windowEvent.key.keysym;

				// Escape to quit (your existing behavior)
				...
				// Optional: ignore auto-repeat if you don't want repeat as multiple presses
				/*if (windowEvent.key.repeat)
					goto after_key;*/

				// Translate to UI code
				if (uint32_t code = UiAsciiFromSDL(ks)) {
					// input controller API expects the code as uint32_t
					inputController.OnKeyJustPressed(code, mod);
				}
			//after_key:;
			}
			else if(SDL_KEYUP == windowEvent.type)
			{
				const SDL_Keysym& ks = windowEvent.key.keysym;
				if (uint32_t code = UiAsciiFromSDL(ks)) {
					// your API takes ASCII enum here; cast is fine since values match
					inputController.OnKeyRelease(static_cast<ASCII>(code));
				}
			}
			else if(SDL_MOUSEBUTTONDOWN == windowEvent.type)
			{
				...
				if(windowEvent.button.clicks == 1)
				  inputController.OnMousePress(windowEvent.motion.x - viewport_offset.x,
					...
					inputController.OnMouseDoublePress(windowEvent.motion.x - viewport_offset.x,
																						 windowEvent.motion.y - viewport_offset.y,
																						 windowEvent.button.button == SDL_BUTTON_LEFT,
																						 mod);
			}
			...
			else if (SDL_DROPFILE == windowEvent.type)
			{
				char* dropped_path = windowEvent.drop.file;
				std::string dropped_file_path = dropped_path;

				int mx_global, my_global;
				SDL_GetGlobalMouseState(&mx_global, &my_global);

				int win_x, win_y;
				SDL_GetWindowPosition(window, &win_x, &win_y);

				// Final drop position relative to SDL window:
				int drop_x = mx_global - win_x;
				int drop_y = my_global - win_y;

				inputController.OnDropEvent(drop_x - viewport_offset.x,
																		drop_y - viewport_offset.y,
																		dropped_file_path);
...
void dbGLWindowSDL::OnDockSpace()
{
	...
	if (io.ConfigFlags & ImGuiConfigFlags_DockingEnable)
	{
		...
		if (std::shared_ptr<eObject> obj = mainContext->GetFocusedObject(); mainContext->UseGizmo() && obj)
		{
		...
		else if (!mainContext->GetParticleSystems().empty() && mainContext->UseGizmo())
		{
			glm::mat4 view = mainContext->GetMainCameraViewMatrix();
			glm::mat4 projection = mainContext->GetMainCameraProjectionMatrix();
			glm::mat4 transform = mainContext->GetParticleSystems()[0]->GetTransform()->getModelMatrix();
			ImGuizmo::Manipulate(&view[0][0], &projection[0][0], (ImGuizmo::OPERATION)mainContext->CurGizmoType(), ImGuizmo::LOCAL, &transform[0][0]);
			...
			if (ImGuizmo::IsUsing())
			{
				glm::vec3 scale; glm::quat rotation; glm::vec3 translation; glm::vec3 sqew; glm::vec4 perspective;
				glm::decompose(transform, scale, rotation, translation, sqew, perspective);
				mainContext->GetParticleSystems()[0]->GetTransform()->setTranslation(translation);
				mainContext->GetParticleSystems()[0]->GetTransform()->setRotation(rotation);
				mainContext->GetParticleSystems()[0]->GetTransform()->setScale(scale);
			}
		}
