...
namespace dbb
{
	...
	const TCPConnection& TCPConnection::operator=(const TCPConnection& _other)
	{
		...
		StringMessageRecieved = _other.StringMessageRecieved; //copy std::function bad for dangling pointers @todo
		...
		std::memcpy(m_buffer, _other.m_buffer, dbb::g_max_packetSize);
		return *this;
	}
	...
	std::pair<size_t, std::string> TCPConnection::Receive()
	{
		...
		if (m_recv_pack.task == dbb::Packet::PacketTask::ProcessPacketSize)
		{
			...
			else if (bytesReceived == SOCKET_ERROR)
			{
				...
				error = "Recv error: " + std::to_string(err);
			}
		...
		{
			bytesReceived = recv(m_socket.GetHandle(), (char*)&m_buffer + m_extraction_offset, m_packet_size - m_extraction_offset, 0); //@todo no &
			if (bytesReceived == 0)
				error = "Recv==0";
			else if (bytesReceived == SOCKET_ERROR)
			{
