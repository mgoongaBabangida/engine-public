...
bool Client::Connect(dbb::IPEndPoint ip)
{
   ...
   if (m_socket.Create(); m_socket.GetSocketError() == 0)
   {
       ...
       {
         WSAPOLLFD fd = { m_socket.GetHandle(), POLLWRNORM, 0 };
         if (WSAPoll(&fd, 1, 5000) > 0 && (fd.revents & POLLWRNORM))  // 5-second timeout
         {
         ...
         {
           base::Log("Connection timed out or failed after WSAEWOULDBLOCK");
         }
       }
       else
       {
